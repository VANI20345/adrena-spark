...
export const PollPost: React.FC<PollPostProps> = ({ postId }) => {
  ...
  const loadPollData = async () => {
    try {
      ...
      const total = pollOptions.reduce((sum, opt) => sum + opt.votes_count, 0);

      // Parallel fetch: user vote check (if logged in) + voters data
      let userVoteData: { option_id: string } | null = null;
      let allVotesData: { option_id: string; user_id: string }[] = [];

      // Only check user vote if logged in
      if (user) {
      ...
      // Fetch voters if there are votes
      if (total > 0) {
        const optionIds = pollOptions.filter(o => o.votes_count > 0).map(o => o.id);
      ...
      if (allVotesData.length > 0) {
        const userIds = [...new Set(allVotesData.map(v => v.user_id))] as string[];
