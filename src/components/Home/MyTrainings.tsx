              ...
              <CarouselItem key={training.id} className="pl-4 basis-full sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 h-full border-transparent hover:border-[hsl(var(--brand-purple)/0.3)]">
                    ...
                    <div className="space-y-2">
                      ...
                      {training.start_date && (
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          <span>{formatEventDate(training.start_date, language === 'ar', 'd MMM yyyy')}</span>
                        </div>
                      )}
                  ...
                  <CardFooter className="px-4 pb-4">
                    <Button asChild className="w-full text-white hover:opacity-90" size="sm" style={{ backgroundColor: '#782097' }}>
                      <Link to={`/service/${training.id}`}>{language === 'ar' ? 'عرض التفاصيل' : 'View Details'}</Link>
                    </Button>
                  </CardFooter>
                </Card>
              </CarouselItem>
