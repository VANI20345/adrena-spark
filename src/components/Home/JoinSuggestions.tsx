...
const JoinSuggestions = () => {
  ...
  useEffect(() => {
    const fetchSuggestedGroups = async () => {
      try {
        // Fetch all active groups first (works for anon + authenticated)
        ...
        // Fallback: recompute member counts from group_memberships in case current_members is stale
        if (rows.length > 0) {
          ...
          rows.forEach((g: any) => {
            const real = counts[g.id] ?? 0;
            if (real > (g.current_members || 0)) g.current_members = real;
          });
        }
        setGroups(rows);
      } catch (error) {
  ...
  return (
    <section className="py-12">
      <div className="container mx-auto px-4">
        ...
        <Carousel
          opts={{ align: "start", loop: true, direction: language === 'ar' ? 'rtl' : 'ltr', dragFree: false, containScroll: 'trimSnaps' }}
          className="w-full"
        >
          <CarouselContent className="-ml-2 sm:-ml-4">
            {groups.map((group) => (
              <CarouselItem key={group.id} className="pl-2 sm:pl-4 basis-[85%] sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                ...
                <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 h-full border-transparent hover:border-[hsl(var(--brand-lime)/0.35)]">
                  ...
                  <CardFooter className="px-4 pb-4">
                    <Button asChild className="w-full text-[#1a1a1a] hover:opacity-90 font-semibold" size="sm" style={{ backgroundColor: '#ccea00' }}>
