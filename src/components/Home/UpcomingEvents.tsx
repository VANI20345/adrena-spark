...
import { getEventTag, getEventPriceLabel } from '@/utils/eventStatus';
              ...
              <div key={event.id} className="flex-shrink-0 w-[280px] sm:w-[300px] md:w-[320px]">
                <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 h-full flex flex-col border-transparent hover:border-[hsl(var(--brand-teal)/0.4)]">
                  <div className="relative h-44">
                    ...
                    {(() => {
                      const tag = getEventTag(event, isRTL);
                      return (
                        <div className={`absolute top-3 ${isRTL ? 'left-3' : 'right-3'} flex flex-col items-end gap-1`}>
                          <Badge variant={tag.variant} className="text-xs">{tag.label}</Badge>
                    ...
                    <div className="absolute bottom-3 right-3">
                      <div className="bg-background/90 backdrop-blur-sm rounded-lg px-2.5 py-1">
                        <span className="text-xs font-bold" style={{ color: '#00c6b5' }}>
                          {getEventPriceLabel(event.price, isRTL)}
                        </span>
                      </div>
                    </div>
                  </div>
                  <CardContent className="p-4 flex-1">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span>{formatEventDate(event.start_date, isRTL, 'd MMM yyyy')}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Clock className="w-3.5 h-3.5 flex-shrink-0" />
                        <span>{formatEventTime(event.start_date, isRTL)}</span>
                      </div>
                      {event.max_attendees && (
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <CardFooter className="px-4 pb-4 pt-0">
                    <Button asChild className="w-full text-white hover:opacity-90" size="sm" style={{ backgroundColor: '#00c6b5' }}>
