...
const TransactionHistory = ({ userId, userRole }: TransactionHistoryProps) => {
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      ...
      case 'refund':
      case 'bonus':
        return <ArrowDownLeft className="h-5 w-5 text-blue-600 dark:text-blue-400" />;
      case 'payment':
      case 'commission':
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge variant="default" className="bg-success text-success-foreground">{t('wallet.completed')}</Badge>;
      case 'pending':
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${
              totalIncome - totalExpenses >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-destructive'
            }`}>
          ...
          <div className="space-y-3">
            {filteredTransactions.length === 0 ? (
              ...
              filteredTransactions.map((transaction) => (
                <div
                  key={transaction.id}
                  className={`flex items-center justify-between p-4 border rounded-xl transition-all duration-200 hover:shadow-md ${getTransactionStyle(transaction.type)} ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                  <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse text-right' : 'text-left'}`}>
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center ${
                      ['earning', 'refund', 'bonus'].includes(transaction.type) 
                    ...
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      ...
                      <div className={`flex items-center gap-2 text-sm text-muted-foreground ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        {transaction.reference_id && (
                          <>
                            <span>•</span>
                            <span className="text-xs">{transaction.reference_id.substring(0, 8)}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    {(() => {
                      ...
                      const sign = isIncome ? '+' : '-';
                      return (
                        <span className={`font-bold text-lg ${isIncome ? 'text-blue-600 dark:text-blue-400' : 'text-destructive'}`}>
