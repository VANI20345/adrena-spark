  ...
  useEffect(() => {
    const checkSuspension = async () => {
      ...
      const { data: suspension } = await supabase
        ...
        .select('is_active, reason, suspended_until, suspended_at')
        .eq('user_id', user.id)
        .eq('is_active', true)
        .maybeSingle();
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newSuspension = payload.new as any;
          if (newSuspension?.is_active) {
            setSuspensionInfo({
              suspended: true,
              suspension_reason: newSuspension.reason,
              suspended_until: newSuspension.suspended_until,
              suspended_at: newSuspension.suspended_at
            });
          } else {
            setSuspensionInfo(null);
          }
