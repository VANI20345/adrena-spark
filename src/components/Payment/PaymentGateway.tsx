...
const PaymentGateway: React.FC<PaymentGatewayProps> = ({
  ...
  const handlePayment = async () => {
    ...
    console.log('[PaymentGateway] Starting payment process...');
    console.log('[PaymentGateway] booking_id:', booking_id);
    console.log('[PaymentGateway] amount:', amount);
    console.log('[PaymentGateway] bookingType:', bookingType);
    
    if (!booking_id) {
      console.error('[PaymentGateway] Missing booking_id!');
      onPaymentFailure('معرف الحجز مفقود - يرجى إعادة المحاولة');
      toast({
        title: 'خطأ في الحجز',
        description: 'معرف الحجز مفقود. يرجى العودة والمحاولة مرة أخرى.',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      if (!session) {
        console.error('[PaymentGateway] No session found');
        throw new Error('يرجى تسجيل الدخول أولاً');
      }
      
      console.log('[PaymentGateway] Session found, user:', session.user.id);
      ...
      console.log('[PaymentGateway] Calling process-payment with:', {
        ...paymentPayload,
        source: { ...paymentPayload.source, number: '****', cvc: '***' }
      });
      ...
      console.log('[PaymentGateway] process-payment response:', { data, error });
