...
export const FloatingTicketButton: React.FC<FloatingTicketButtonProps> = ({ className }) => {
  ...
  const { data: unreadData, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Count tickets where last message is not from current user and hasn't been read locally
      let unreadCount = 0;
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            ...
            >
              {/* Single smooth pulse ring - starts small and expands smoothly */}
              <AnimatePresence>
                {hasNewMessages && (
                  <motion.span 
