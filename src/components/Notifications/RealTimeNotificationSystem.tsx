...
const RealTimeNotificationSystem: React.FC<RealTimeNotificationSystemProps> = ({
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // Silent fail - notifications will be empty
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Header */}
      <Card>
        <CardHeader className={isRTL ? 'text-right' : 'text-left'}>
          <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              {unreadCount > 0 && (
                <Button variant="outline" size="sm" onClick={markAllAsRead} className={isRTL ? 'flex-row-reverse' : ''}>
