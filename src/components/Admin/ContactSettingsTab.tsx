    ...
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className={`flex items-start justify-between gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
        <div className={isRTL ? 'text-right' : 'text-left'}>
          ...
          <p className={isRTL ? 'text-muted-foreground text-right' : 'text-muted-foreground text-left'}>
            {isRTL ? 'إدارة معلومات التواصل التي تظهر في صفحة اتصل بنا' : 'Manage contact information shown on the Contact Us page'}
          </p>
        </div>
        <Button onClick={handleSave} disabled={saving} className={isRTL ? 'flex-row-reverse self-start' : 'self-start'}>
          {saving ? (
      ...
      <Card>
        ...
        <CardContent className={`grid gap-4 md:grid-cols-2 ${isRTL ? 'text-right' : 'text-left'}`}>
