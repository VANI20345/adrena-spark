...
const CommissionSettingsTab = ({ isRTL = false }: CommissionSettingsTabProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Send notifications for changed rates
      const changes: string[] = [];
      if (currentRates.events !== rates.events) {
      ...
      if (currentRates.services !== rates.services) {
        changes.push(language === 'ar'
      ...
      if (currentRates.training !== rates.training) {
        changes.push(language === 'ar'
      ...
      // Send notification to all providers if there are changes
      if (changes.length > 0) {
