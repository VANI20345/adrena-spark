...
const CommissionSettingsTab = ({ isRTL = false }: CommissionSettingsTabProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If wallet hold percent changed, recalculate existing active holds
      const holdChanged = originalHoldPercentRef.current !== rates.walletHoldPercent;
      if (holdChanged) {
        try {
          const { data: rpcResult, error: rpcError } = await supabase.rpc('recalculate_active_holds' as any, {
            p_new_hold_pct: rates.walletHoldPercent,
          });
          if (rpcError) {
            console.warn('recalculate_active_holds RPC failed (function may not exist yet):', rpcError);
          } else if ((rpcResult as any)?.updated_count > 0) {
            toast.info(
              language === 'ar'
                ? `تم تحديث ${(rpcResult as any).updated_count} هولد نشط بالنسبة الجديدة (${rates.walletHoldPercent}%)`
                : `Updated ${(rpcResult as any).updated_count} active hold(s) with new rate (${rates.walletHoldPercent}%)`
            );
          }
          originalHoldPercentRef.current = rates.walletHoldPercent;
        } catch (holdErr) {
          console.warn('Hold recalculation failed:', holdErr);
        }
      }
