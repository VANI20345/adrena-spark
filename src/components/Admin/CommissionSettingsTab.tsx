...
const CommissionSettingsTab = ({ isRTL = false }: CommissionSettingsTabProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updates = [
        { key: 'commission_events', value: { percentage: rates.events }, description: 'Commission percentage for events', updated_at: new Date().toISOString() },
        { key: 'commission_services', value: { percentage: rates.services }, description: 'Commission percentage for services', updated_at: new Date().toISOString() },
        { key: 'commission_training', value: { percentage: rates.training }, description: 'Commission percentage for training', updated_at: new Date().toISOString() },
      ];
      ...
      // Super admins + admins always get notified; attendees for events, providers for services/training
      type AppRole = 'admin' | 'attendee' | 'organizer' | 'provider' | 'super_admin';
      const targetRoles: AppRole[] = [];
      if (eventsChanged) {
        // Events affect attendees
        targetRoles.push('attendee');
      }
      if (servicesChanged || trainingChanged) {
        // Services/training affect providers
        targetRoles.push('provider');
      }
      // Always include admins and super_admins
      ...
      // Send targeted notifications if there are changes
      if (changes.length > 0) {
        // Fetch user IDs for targeted roles - remove duplicates
  ...
  const CommissionInput = ({
    ...
    <div className={`p-4 border rounded-lg space-y-3 ${isRTL ? 'text-right' : 'text-left'}`}>
      <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <Input
            ...
            dir="ltr"
          />
          <span className={`text-lg font-medium ${isRTL ? 'text-right' : 'text-left'}`}>%</span>
        </div>
      </div>
      <p className={`text-xs text-muted-foreground bg-muted p-2 rounded ${isRTL ? 'text-right' : 'text-left'}`}>
