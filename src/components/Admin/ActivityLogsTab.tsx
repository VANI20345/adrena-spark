                      ...
                      {log.details && typeof log.details === 'object' ? (
                        <div className="space-y-1 text-sm">
                          {log.details.user_name && (
                            <div><span className="font-medium">المستخدم:</span> {log.details.user_name}</div>
                          )}
                          {log.details.user_email && (
                            <div><span className="font-medium">البريد:</span> {log.details.user_email}</div>
                          )}
                          {log.details.event_title && (
                            <div><span className="font-medium">الفعالية:</span> {log.details.event_title}</div>
                          )}
                          {log.details.service_name && (
                            <div><span className="font-medium">الخدمة:</span> {log.details.service_name}</div>
                          )}
                          {log.details.reason && (
                            <div><span className="font-medium">السبب:</span> {log.details.reason}</div>
                          )}
                          {log.details.duration_days && (
                            <div><span className="font-medium">المدة:</span> {log.details.duration_days} يوم</div>
                          )}
                          {log.details.suspended_until && (
                            <div><span className="font-medium">حتى:</span> {new Date(log.details.suspended_until).toLocaleDateString('ar-SA')}</div>
                          )}
                          {log.details.new_role && (
                            <div><span className="font-medium">الدور الجديد:</span> {log.details.new_role}</div>
                          )}
                          {log.details.comment && (
                            <div><span className="font-medium">تعليق:</span> {log.details.comment}</div>
                          )}
                        </div>
                      ) : (
