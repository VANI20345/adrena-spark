      ...
      <CardHeader>
        <div className="flex items-center justify-between gap-3 w-full">
          <CardTitle className="flex items-center gap-2">
            {isRTL ? (
              <>
                <span>تصنيفات الفعاليات والاهتمامات</span>
                <Tags className="w-5 h-5" />
              </>
            ) : (
              <>
                <Tags className="w-5 h-5" />
                <span>Event Categories & Interests</span>
              </>
            )}
          </CardTitle>
                    ...
                    <TableRow key={category.id}>
                      <TableCell className={`font-medium ${isRTL ? 'text-right' : 'text-left'}`}>
                        <div className="flex flex-col">
                          <span>{isRTL ? category.name_ar : category.name}</span>
                          <span className="text-xs text-muted-foreground">{isRTL ? category.name : category.name_ar}</span>
                        </div>
                      </TableCell>
