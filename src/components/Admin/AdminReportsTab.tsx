...
const AdminReportsTab = () => {
  ...
  const fetchReportData = async () => {
    ...
    const [users, events, services, bookings, serviceBookings] = await Promise.all([
      ...
      supabase.from('services').select('created_at, status')
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString()),
      supabase.from('bookings').select('created_at, total_amount, platform_commission, status')
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString()),
      supabase.from('service_bookings').select('created_at, total_amount, platform_commission, status')
        .gte('created_at', startDate.toISOString())
        .lte('created_at', endDate.toISOString())
    ]);
    ...
    (bookings.data || []).forEach(b => addToGroup(new Date(b.created_at), 'booking', b.platform_commission ?? 0));
    (serviceBookings.data || []).forEach(b => addToGroup(new Date(b.created_at), 'booking', b.platform_commission ?? 0));
              ...
              <TableBody>
                {!reportData || reportData.length === 0 ? (
                  ...
                  reportData.map((row, index) => (
                    <TableRow key={index}>
                      <TableCell className={`font-medium super-admin-table-cell ${isRTL ? 'text-right' : 'text-left'}`}>{row.period}</TableCell>
