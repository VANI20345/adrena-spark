...
export const GroupManagementTab = () => {
  ...
  const loadGroups = async () => {
    setLoading(true);
    try {
      // Fetch all groups with optional event data (left join approach)
      ...
      // Fetch event data separately for groups that have event_id
      const groupsWithEventId = (eventData || []).filter(g => g.event_id);
      ...
      // Merge events with groups
      const eventDataWithEvents = (eventData || []).map(group => ({
        ...group,
        events: group.event_id ? eventsMap[group.event_id] : null
      }));

      const groupsWithAdmins = await Promise.all(
        (eventDataWithEvents || []).map(async (group) => {
