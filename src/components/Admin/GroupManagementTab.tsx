...
export const GroupManagementTab = () => {
  ...
  const loadGroups = async () => {
    setLoading(true);
    try {
      // تحميل مجموعات الفعاليات - إصلاح العلاقة المتعددة
      const { data: eventData, error: eventError } = await supabase
        .from('event_groups')
        .select(`
          *,
          events!event_groups_event_id_fkey(title_ar, end_date)
        `)
      ...
      // تحميل مجموعات المناطق من event_groups
  ...
  const handleDeleteGroup = async (groupId: string, isRegional: boolean = false) => {
    ...
    try {
      const table = 'event_groups'; // All groups are in event_groups now
      const { error } = await supabase
        .from(table)
        .delete()
        .eq('id', groupId);
