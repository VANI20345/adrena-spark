...
import { PaymentHoldsSection } from './PaymentHoldsSection';
import PlatformVATSettings from './PlatformVATSettings';
import { FinancialAuditLogSection } from './FinancialAuditLogSection';
import { HoldDurationSettings } from './HoldDurationSettings';
...
export const FinancialDashboardTab = () => {
  ...
  const { data: financialData, isLoading } = useSupabaseQuery({
    ...
    queryFn: async () => {
      ...
      const [bookingsResult, serviceBookingsResult, walletTransactionsResult] = await Promise.all([
        supabase
          .from('bookings')
      ...
      const eventRevenue = bookings.reduce((sum, b) => sum + (Number(b.total_amount) || 0), 0);
      const serviceRevenue = serviceBookings.reduce((sum, b) => sum + (Number(b.total_amount) || 0), 0);
      const totalRevenue = eventRevenue + serviceRevenue;

      const eventCommission = bookings.reduce((sum, b) => sum + (Number(b.platform_commission) || 0), 0);
      const serviceCommission = serviceBookings.reduce((sum, b) => sum + (Number(b.platform_commission) || 0), 0);
      const totalPlatformCommission = eventCommission + serviceCommission;

      const eventVatOnCommission = bookings.reduce((sum, b) => sum + (Number(b.vat_on_commission) || 0), 0);
      const serviceVatOnCommission = serviceBookings.reduce((sum, b) => sum + (Number(b.vat_on_commission) || 0), 0);
      const totalVatOnCommission = eventVatOnCommission + serviceVatOnCommission;

      const eventProviderEarnings = bookings.reduce((sum, b) => sum + (Number(b.provider_earnings) || 0), 0);
      const serviceProviderEarnings = serviceBookings.reduce((sum, b) => sum + (Number(b.provider_earnings) || 0), 0);
      ...
      const totalTopups = walletTransactions
        .filter(t => ['credit', 'deposit'].includes(String(t.type)))
        .reduce((s, t) => s + Math.abs(Number(t.amount) || 0), 0);
      const totalWithdrawals = walletTransactions
        .filter(t => ['debit', 'withdraw'].includes(String(t.type)))
        .reduce((s, t) => s + Math.abs(Number(t.amount) || 0), 0);
      ...
      const transactionLog = [
        ...bookings.map(b => ({
          ...
          platform_commission: Number(b.platform_commission) || 0,
          status: b.status,
        ...
        ...walletTransactions.map(t => ({
          ...
          source: ['credit', 'deposit'].includes(String(t.type)) ? 'wallet_topup' as const : 'wallet_withdrawal' as const,
        })),
      ]
      ...
      return {
        ...
        totalWithdrawals,
        commissionRates,
        transactionsCount: transactionLog.length,
  ...
  const formatCurrency = (amount: number) => {
    return `${amount.toLocaleString(isRTL ? 'ar-SA' : 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${isRTL ? 'ريال' : 'SAR'}`;
  };

  const getTransactionLabel = (source: 'event' | 'service' | 'wallet_topup' | 'wallet_withdrawal') => {
    switch (source) {
      case 'event':
        return isRTL ? 'حجز فعالية' : 'Event Booking';
      case 'service':
        return isRTL ? 'حجز خدمة' : 'Service Booking';
      case 'wallet_topup':
        return isRTL ? 'شحن محفظة' : 'Wallet Top-up';
      case 'wallet_withdrawal':
        return isRTL ? 'سحب محفظة' : 'Wallet Withdrawal';
      default:
        return '—';
    }
  };
    ...
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <Select value={period} onValueChange={setPeriod}>
          <SelectTrigger className={`w-full sm:w-48 ${isRTL ? 'flex-row-reverse text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <Card key={index}>
            <CardContent className="pt-6">
              <div className="flex items-start justify-between gap-3">
                <div className={`flex items-start gap-3 min-w-0 flex-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <div className={`p-3 rounded-full shrink-0 ${stat.bgColor}`}>
                    <stat.icon className={`h-5 w-5 ${stat.color}`} />
                  </div>
                  <div className={`min-w-0 ${isRTL ? 'text-right' : 'text-left'}`}>
                    <p className="text-sm font-medium text-muted-foreground">{stat.title}</p>
                    <p className="text-2xl font-bold mt-1 tabular-nums" dir="ltr">{stat.value}</p>
      ...
      {/* Payment Holds (Escrow) Section */}
      <PaymentHoldsSection />

      {/* Hold Duration Setting */}
      <HoldDurationSettings />

      {/* ZATCA / VAT Settings */}
      <PlatformVATSettings />

      {/* Financial Audit Log */}
      <FinancialAuditLogSection />
