  ...
  return (
    <Card className="border-2 border-amber-200 dark:border-amber-900/40">
      <CardHeader className={isRTL ? 'text-right' : 'text-left'}>
        ...
        <CardDescription>
          {isRTL
            ? `يتم تحرير ${availablePercent}% من أرباح المزود فوراً للمحفظة، و${holdPercent}% تبقى محجوزة 72 ساعة بعد انتهاء الفعالية. عند وجود نزاع يتم تجميد الإفراج تلقائياً.`
            : `${availablePercent}% of provider earnings are released immediately to wallet; ${holdPercent}% are held for 72h after event end. Disputes auto-freeze release.`}
        </CardDescription>
      </CardHeader>
                  ...
                  {isLoading ? (
                    ...
                    holds.map((h) => {
                      ...
                      const releaseTooltip = isDispute
                        ...
                        : isPending
                          ? (isRTL ? `متاح بعد ${new Date(h.hold_until).toLocaleString(isRTL ? 'ar-SA' : 'en-US')}` : `Available after ${new Date(h.hold_until).toLocaleString('en-US')}`)
                          : '';
                      return (
                        <TableRow key={h.id}>
                      ...
                      <div className="p-3 bg-red-500/10 rounded-lg border border-red-500/20 space-y-2">
                        ...
                        <p className="text-xs text-red-600/80 leading-relaxed">
                          {isDispute
                            ...
                            : (isRTL 
                                ? `المبلغ لا يزال قيد الحجز الزمني ولا ينتهي القفل إلا بتاريخ ${new Date(releaseDialog.hold_until).toLocaleString(isRTL ? 'ar-SA' : 'en-US')}.` 
                                : `Funds are locked until ${new Date(releaseDialog.hold_until).toLocaleString('en-US')}.`)}
                        </p>
