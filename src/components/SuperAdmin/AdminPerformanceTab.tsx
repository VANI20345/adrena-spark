  ...
  const { data: performanceData = [], isLoading, error, refetch } = useSupabaseQuery({
    ...
    queryFn: async (): Promise<AdminPerformance[]> => {
      try {
        ...
        // Only keep admins that still have BOTH a current role AND a profile.
        // This automatically excludes deleted accounts and demoted admins.
        const validAdminIds = [...new Set(adminIds)].filter(id =>
        ...
        const performance: AdminPerformance[] = validAdminIds.map(adminId => {
          ...
          const displayName = (profile?.full_name && profile.full_name.trim()) || null;
        ...
        return performance
          .filter(item => item.admin.full_name || item.admin.avatar_url || item.admin.account_created)
          .sort((a, b) => b.sinceCreation.totalActions - a.sinceCreation.totalActions);
      } catch (err) {
            ...
            <TabsList className="grid w-full grid-cols-3" dir={isRTL ? 'rtl' : 'ltr'}>
              {isRTL ? (
                <>
                  ...
                  <TabsTrigger value="sinceCreation" className="flex items-center gap-2">
            ...
            return (
              ...
              >
                <CardContent className="pt-6">
                  ...
                  <div className={`grid grid-cols-3 gap-2 mt-4 ${isRTL ? 'text-right' : 'text-center'}`}>
                    <div className={`p-2 bg-green-100 dark:bg-green-900/20 rounded ${isRTL ? 'text-right' : 'text-center'}`}>
                      <p className="text-lg font-bold text-green-600">{totalApproved}</p>
                      <p className="text-xs text-muted-foreground">{t.approved}</p>
                    </div>
                    <div className={`p-2 bg-red-100 dark:bg-red-900/20 rounded ${isRTL ? 'text-right' : 'text-center'}`}>
                      <p className="text-lg font-bold text-red-600">{totalRejected}</p>
                      <p className="text-xs text-muted-foreground">{t.rejected}</p>
                    </div>
                    <div className={`p-2 bg-blue-100 dark:bg-blue-900/20 rounded ${isRTL ? 'text-right' : 'text-center'}`}>
