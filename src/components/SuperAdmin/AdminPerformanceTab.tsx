...
export const AdminPerformanceTab = () => {
  ...
  const { data: admins = [], isLoading } = useSupabaseQuery({
    ...
    queryFn: async () => {
      ...
      const adminStats = adminIds.map(adminId => {
  ...
  const selectedAdminData = admins.find(a => a.user_id === selectedAdmin);
              ...
              <TabsContent value="activity" className="mt-4">
                <div className="rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {selectedAdminData.logs.slice(0, 20).map((log: any, index: number) => (
                        <TableRow key={index}>
                          <TableCell>
