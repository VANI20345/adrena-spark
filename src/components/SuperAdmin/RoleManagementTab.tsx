  ...
  const { data: usersWithRoles = [], isLoading, error, refetch } = useSupabaseQuery({
    ...
    queryFn: async (): Promise<UserWithRole[]> => {
      try {
        ...
        // Get all roles directly (fallback if edge function doesn't include them)
        ...
        (roles || []).forEach(r => {
          // Prioritize higher-privilege roles if a user somehow has multiple
          const existing = roleMap.get(r.user_id);
          const priority: Record<string, number> = { super_admin: 4, admin: 3, provider: 2, attendee: 1 };
        ...
        if (!edgeFnError && edgeFnData?.users) {
          ...
          return edgeFnData.users.map((u: any) => {
            ...
            // Prefer roles from edge function payload if available, fallback to roleMap
            let resolvedRole: string | undefined;
            if (Array.isArray(u.user_roles) && u.user_roles.length > 0) {
              const priority: Record<string, number> = { super_admin: 4, admin: 3, provider: 2, attendee: 1 };
              resolvedRole = u.user_roles
                .map((r: any) => r.role)
            ...
            return {
              ...
              phone: contactMap.get(userId)?.phone || u.phone || null,
              email: contactMap.get(userId)?.email || u.email || null,
              created_at: u.created_at,
              role: resolvedRole || roleMap.get(userId) || 'attendee',
            };
          });
        }
      ...
      <Card>
        <CardHeader className={isRTL ? 'text-right' : 'text-left'}>
          <div className="flex items-start justify-between gap-4">
            ...
            <div className={`flex items-center gap-2 rounded-lg border bg-muted/40 px-4 py-2 self-start ${isRTL ? 'flex-row-reverse' : ''}`}>
              <User className="h-4 w-4 text-primary" />
              <span className="text-sm text-muted-foreground">{t.totalUsers}</span>
              <span className="text-lg font-bold text-primary">{sharedTotalUsers}</span>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className={`flex flex-col sm:flex-row gap-4 mb-6 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger className={`w-full sm:w-48 ${isRTL ? 'text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((u) => (
                    <TableRow key={u.user_id}>
                      ...
                      <TableCell className={isRTL ? 'text-right' : 'text-left'}>{getRoleBadge(u.role)}</TableCell>
