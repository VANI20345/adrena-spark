...
export const RoleManagementTab = () => {
  ...
  const handleRoleChange = async () => {
    ...
    if (newRole === 'super_admin') {
      // Additional check - verify current user is super_admin
  ...
  const handleRevokeRole = async (userId: string, userName: string, currentRole: string) => {
  ...
  // Check if the selected user is a Super Admin (for UI protection)
                    ...
                    <TableRow key={u.user_id}>
                      ...
                      <TableCell>
                        <div className={`flex gap-2 ${isRTL ? 'justify-start' : 'justify-end'}`}>
                          {/* Disable edit for Super Admin users */}
                          <Button
                            size="sm"
                            variant="outline"
                          ...
                          {/* Only show revoke for admin (not super_admin) */}
                          {u.role === 'admin' && (
                            <Button
                              size="sm"
                              variant="destructive"
