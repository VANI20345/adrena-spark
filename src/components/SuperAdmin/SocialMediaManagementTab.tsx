...
import { Save, Loader2, Globe, Youtube, Instagram, Facebook, Linkedin, Info, ExternalLink, CheckCircle } from 'lucide-react';
...
const platforms: SocialMediaPlatform[] = [
  ...
  { id: 'twitter', name: 'Twitter (X)', nameAr: 'تويتر (X)', icon: XIcon, placeholder: 'https://x.com/username', color: 'text-foreground' },
  { id: 'facebook', name: 'Facebook', nameAr: 'فيسبوك', icon: Facebook, placeholder: 'https://facebook.com/page', color: 'text-blue-600' },
  { id: 'linkedin', name: 'LinkedIn', nameAr: 'لينكد إن', icon: Linkedin, placeholder: 'https://linkedin.com/company/name', color: 'text-blue-700' },
];
...
export const SocialMediaManagementTab = () => {
  ...
  const translations = {
    ar: {
      ...
      description: 'إضافة وتعديل وإظهار/إخفاء روابط وأيقونات وسائل التواصل الاجتماعي في Footer وصفحة التواصل',
      saveChanges: 'حفظ التغييرات',
      saving: 'جاري الحفظ...',
      successSave: 'تم حفظ الإعدادات بنجاح وستظهر التغييرات في الفوتر وصفحة التواصل',
      errorSave: 'حدث خطأ أثناء الحفظ',
      errorLoad: 'حدث خطأ أثناء التحميل',
      url: 'الرابط',
      showIcon: 'إظهار الأيقونة',
      preview: 'معاينة الفوتر',
      visibleIcons: 'الأيقونات المرئية',
      hiddenIcons: 'الأيقونات المخفية',
      syncNote: 'التغييرات تنعكس مباشرة على Footer الموقع وصفحة التواصل بعد الحفظ',
      previewNote: 'معاينة كيف ستظهر الأيقونات في الفوتر وصفحة التواصل',
      unsavedChanges: 'لديك تغييرات غير محفوظة',
      footerIntegration: 'تكامل الفوتر',
      footerIntegrationDesc: 'الأيقونات المفعلة ستظهر تلقائياً في الفوتر وصفحة التواصل',
    },
    en: {
      title: 'Social Media Management',
      description: 'Add, edit, and show/hide social media links and icons in Footer and Contact page',
      saveChanges: 'Save Changes',
      saving: 'Saving...',
      successSave: 'Settings saved successfully. Changes will appear in Footer and Contact page',
      ...
      syncNote: 'Changes are synced to the website Footer and Contact page after saving',
      previewNote: 'Preview how icons will appear in Footer and Contact page',
      unsavedChanges: 'You have unsaved changes',
      footerIntegration: 'Footer Integration',
      footerIntegrationDesc: 'Enabled icons will automatically appear in the Footer and Contact page',
    },
  };

  const t = translations[language];

  // Store the original settings to compare for unsaved changes
  const [originalSettings, setOriginalSettings] = useState<Record<string, SocialSettings>>({});
  ...
  const loadSettings = async () => {
    try {
      ...
      platforms.forEach(platform => {
        initialSettings[platform.id] = socialMedia[platform.id] || { url: '', visible: false };
      });
      
      setSettings(initialSettings);
      setOriginalSettings(JSON.parse(JSON.stringify(initialSettings))); // Deep copy for comparison
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Reload settings from database to confirm save worked
      const allSettings = await adminService.getSystemSettings();
      const savedSocialMedia = allSettings.social_media || {};
      
      // Update local state with what's actually in DB
      const confirmedSettings: Record<string, SocialSettings> = {};
      platforms.forEach(platform => {
        confirmedSettings[platform.id] = savedSocialMedia[platform.id] || { url: '', visible: false };
      });
      setSettings(confirmedSettings);
      setOriginalSettings(JSON.parse(JSON.stringify(confirmedSettings))); // Update original after successful save
  ...
  const checkForChanges = (newSettings: Record<string, SocialSettings>) => {
  ...
  return (
    <div className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-start justify-between gap-4">
        ...
        <Button onClick={handleSave} disabled={saving || !hasUnsavedChanges} className="self-start">
          {saving ? (
      ...
      <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
        ...
        <AlertDescription className={`text-blue-800 dark:text-blue-300 ${isRTL ? 'text-right' : 'text-left'}`}>
          <strong>{t.footerIntegration}:</strong> {t.footerIntegrationDesc}
        </AlertDescription>
      </Alert>
      ...
      <Card>
          ...
          <CardTitle className="flex items-center gap-2">
            ...
            <CardDescription className={isRTL ? 'text-right' : 'text-left'}>{t.previewNote}</CardDescription>
        </CardHeader>
        <CardContent>
            <div className={`p-4 bg-secondary rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
              <p className="text-sm text-muted-foreground mb-3">
              {language === 'ar' ? 'كما سيظهر في الفوتر:' : 'As it will appear in Footer:'}
            </p>
            <div className={`flex flex-wrap gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              {visiblePlatforms.length > 0 ? (
                visiblePlatforms.map(platform => {
                  ...
                  return (
                    <a
                      ...
                      className={`p-3 rounded-full bg-background hover:bg-muted transition-colors ${platform.color}`}
                ...
                <p className="text-muted-foreground">
                  {language === 'ar' ? 'لا توجد أيقونات مرئية - فعّل أيقونة واحدة على الأقل' : 'No visible icons - enable at least one icon'}
                </p>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {platforms.map(platform => {
          const Icon = platform.icon;
          const platformSettings = settings[platform.id] || { url: '', visible: false };

          return (
            <Card key={platform.id} className={platformSettings.visible ? 'ring-2 ring-green-500/30' : ''}>
              <CardHeader className={isRTL ? 'text-right' : 'text-left'}>
                <CardTitle className="flex items-center gap-2">
                  <Icon className={`h-5 w-5 ${platform.color}`} />
                  {isRTL ? platform.nameAr : platform.name}
                  {platformSettings.visible && (
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <Label className={isRTL ? 'text-right block' : ''}>{t.url}</Label>
                  <Input
                ...
                <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <Label className={isRTL ? 'text-right block' : ''}>{t.showIcon}</Label>
                  <Switch
