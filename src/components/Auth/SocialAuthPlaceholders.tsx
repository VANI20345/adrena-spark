...
export const SocialAuthPlaceholders: React.FC<{ showSeparator?: boolean }> = ({ showSeparator = true }) => {
  ...
  const comingSoon = isRTL ? 'قريباً' : 'Coming soon';
        ...
        <div className="grid grid-cols-2 gap-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <div>
                <Button type="button" variant="outline" className="w-full gap-2 opacity-70 cursor-not-allowed" disabled aria-disabled>
                  <GoogleIcon />
                  {isRTL ? 'جوجل' : 'Google'}
                </Button>
              </div>
