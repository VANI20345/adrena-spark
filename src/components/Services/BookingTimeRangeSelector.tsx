...
export default function BookingTimeRangeSelector({
  ...
  const timeOptions = useMemo(() => {
    ...
    const opts: Array<{ value: string; label: string; minutes: number }> = [];
    for (let m = from; m <= to; m += stepMinutes) {
      const v = minutesToHHMM(m);
      opts.push({ value: v, label: formatTime12h(v), minutes: m });
    }
    return opts;
  }, [availableFrom, availableTo, stepMinutes, language]);
