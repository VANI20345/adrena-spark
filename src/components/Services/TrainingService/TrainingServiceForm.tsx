...
import WeeklyScheduleEditor, { WeeklySchedule } from "../WeeklyScheduleEditor";
import TrainingAvailabilitySelector from "../TrainingAvailabilitySelector";
...
const trainingServiceSchema = z.object({
  ...
  description_ar: z.string().min(20, "الوصف بالعربية يجب أن يكون 20 حرف على الأقل"),
...
const TrainingServiceForm = () => {
  ...
  // Check if at least one slot is scheduled
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (!hasScheduledSlots) {
      toast({ title: "خطأ", description: "يجب إضافة موعد واحد على الأقل في الجدول الأسبوعي", variant: "destructive" });
      return;
    }

    if (!availableFrom) {
      toast({ title: "خطأ", description: "يجب تحديد تاريخ البدء", variant: "destructive" });
      return;
    }

    if (!availableForever && !availableTo) {
      toast({ title: "خطأ", description: "يجب تحديد تاريخ الانتهاء أو اختيار متاح للأبد", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.from('services').insert([{
        ...
        available_forever: availableForever,
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>الحد الأقصى للمتدربين لكل جلسة</FormLabel>
                          <FormControl>
                            <Input 
                              type="number" 
                              min="1"
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-right block">مدة الجلسة</FormLabel>
