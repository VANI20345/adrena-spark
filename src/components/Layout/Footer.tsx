...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time updates for social media settings
    const channel = supabase
      .channel('social-media-settings-realtime')
      .on(
        'postgres_changes',
        {
          event: '*',
          ...
          filter: 'key=eq.social_media'
        },
        (payload: any) => {
          if (payload.new && payload.new.value) {
            setSocialSettings(payload.new.value as SocialMediaSettings);
          } else {
  ...
  const translations = {
    ...
    en: {
      ...
      copyright: '© {year} Hiwaya. All rights reserved.',
    }
  };
  ...
  const renderSocialIcons = () => {
    ...
    if (socialSettings?.instagram?.visible === true) {
      icons.push(
        <a key="instagram" href={socialSettings.instagram.url || 'https://instagram.com'} target="_blank" rel="noopener noreferrer" aria-label="Instagram">
          <Instagram className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.twitter?.visible === true) {
      icons.push(
        <a key="twitter" href={socialSettings.twitter.url || 'https://x.com'} target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)">
          <XIcon className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.tiktok?.visible === true) {
      icons.push(
        <a key="tiktok" href={socialSettings.tiktok.url || 'https://tiktok.com'} target="_blank" rel="noopener noreferrer" aria-label="TikTok">
          <TikTokIcon className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.youtube?.visible === true) {
      icons.push(
        <a key="youtube" href={socialSettings.youtube.url || 'https://youtube.com'} target="_blank" rel="noopener noreferrer" aria-label="YouTube">
          <Youtube className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.facebook?.visible === true) {
      icons.push(
        <a key="facebook" href={socialSettings.facebook.url || 'https://facebook.com'} target="_blank" rel="noopener noreferrer" aria-label="Facebook">
          <Facebook className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.snapchat?.visible === true) {
      icons.push(
        <a key="snapchat" href={socialSettings.snapchat.url || 'https://snapchat.com'} target="_blank" rel="noopener noreferrer" aria-label="Snapchat">
          <SnapchatIcon className={iconClass} />
        </a>
      );
    }
    if (socialSettings?.linkedin?.visible === true) {
      icons.push(
        <a key="linkedin" href={socialSettings.linkedin.url || 'https://linkedin.com'} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <img src={greenLogo} alt="Logo" className="h-10 object-contain" />
            </div>
