    ...
    const loadData = async () => {
      try {
        const [citiesData, interestsData] = await Promise.all([
          ...
          supabase.from('interest_categories').select('*').order('name_ar')
        ]);
                ...
                <div className="space-y-2" dir={isRTL ? 'rtl' : 'ltr'}>
                  ...
                  >
                    ...
                    <SelectContent align={isRTL ? 'end' : 'start'}>
                      {cities.map((city) => (
                        <SelectItem key={city.id} value={city.id} className={isRTL ? 'text-right' : 'text-left'}>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                      ...
                      <Label htmlFor="all" className={`font-normal cursor-pointer ${isRTL ? 'text-right' : ''}`}>
                        {isRTL ? 'الجميع' : 'All'}
                      </Label>
                    </div>
                    <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                      <RadioGroupItem value="male" id="male" />
                      <Label htmlFor="male" className={`font-normal cursor-pointer ${isRTL ? 'text-right' : ''}`}>
                        {isRTL ? 'ذكور فقط' : 'Male only'}
                      </Label>
                    </div>
                    <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                      <RadioGroupItem value="female" id="female" />
                      <Label htmlFor="female" className={`font-normal cursor-pointer ${isRTL ? 'text-right' : ''}`}>
