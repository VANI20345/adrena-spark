...
export const AdmissionFormDialog: React.FC<AdmissionFormDialogProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const admissionData = questions.map((q, i) => ({
        question: q,
      ...
      const result = await requestGroupJoin({
        ...
        admissionAnswers: admissionData,
          ...
          <div className="space-y-3 pt-4 flex-shrink-0">
            ...
            <div className="flex items-center justify-center gap-2">
                {questions.map((_, index) => {
                  // Disable navigation to steps that haven't been unlocked
                  const canGoToStep = index === 0 || answers.slice(0, index).every(a => a.trim().length > 0);
                  return (
                  <button
                    key={index}
