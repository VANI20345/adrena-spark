...
import { getEventTag } from '@/utils/eventStatus';
        ...
        <div className="flex">
          ...
          <CardContent className="p-4 flex-1">
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <Badge variant={tag.variant} className="text-xs shrink-0">
                {tag.label}
              </Badge>
            </div>
            <div className="space-y-1 text-xs text-muted-foreground mb-3">
              <div className="flex items-center gap-1">
                ...
                <span className="line-clamp-1">
                  {toEnglishDigits(format(new Date(event.start_date), 'PPP', { locale: isRTL ? ar : undefined }))}
                </span>
              </div>
            </div>
            <Button
