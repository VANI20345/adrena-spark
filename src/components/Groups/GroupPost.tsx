...
import { Heart, MessageCircle, Send, Trash2, MoreVertical, Play } from 'lucide-react';
import { ReportPostDialog } from './ReportPostDialog';
import { ReportButton } from '@/components/Reports/ReportButton';
import { EnhancedMediaViewerDialog } from './EnhancedMediaViewerDialog';
...
export const GroupPost: React.FC<GroupPostProps> = ({ post, onUpdate, groupId, userRole }) => {
  ...
  const isMediaPost = post.media_urls && post.media_urls.length > 0;
  const canDelete = user && (user.id === post.user_id || userRole === 'owner' || userRole === 'admin');

  const handleLike = async () => {
    if (!user) return;

    // Optimistic update — flip UI immediately, sync DB in background.
    ...
    try {
      ...
      // Don't call onUpdate() — refetch causes a visible flash and undoes the
      // smooth optimistic transition. Counts stay accurate from local state.
    } catch (error) {
      // Revert on failure
      setIsLiked(wasLiked);
  ...
  const loadComments = async () => {
    ...
    setComments((data || []).map((c: any) => ({ ...c, profiles: profilesMap[c.user_id] || null })));
  };
  ...
  const toggleComments = () => {
    // For media posts, open the enhanced lightbox with comments
    if (isMediaPost) {
    ...
    // For text posts, show inline comments
    if (!showComments) {
      loadComments();
    }
  ...
  const handleLikeFromLightbox = () => {
    // Sync like state when toggled from lightbox
        ...
        {post.media_urls && post.media_urls.length > 0 && (
          <>
            ...
            <EnhancedMediaViewerDialog
              ...
              onLikeToggle={handleLikeFromLightbox}
              onUpdate={onUpdate}
              userRole={userRole}
            />
          </>
