...
export const CreateGroupDialog = ({ events = [], onGroupCreated }: CreateGroupDialogProps) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // إنشاء المجموعة - سيتم تعيين المشرفين تلقائياً عبر trigger
      const { error } = await supabase.from('event_groups').insert({
        ...
        created_by: user.id,
        current_members: 0 // سيتم زيادته عند إضافة المشرفين عبر trigger
      });
      ...
      toast({
        title: 'تم بنجاح!',
        description: 'تم إنشاء المجموعة وتعيين المشرفين تلقائياً'
      });
