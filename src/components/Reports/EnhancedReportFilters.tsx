    ...
    <div className={`flex flex-wrap items-end gap-4`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className={`flex-1 min-w-[150px]`}>
        ...
        >
          ...
          <SelectContent className={isRTL ? 'text-right' : 'text-left'}>
            {years.map((year) => (
              <SelectItem key={year} value={year.toString()} className={isRTL ? 'text-right' : 'text-left'}>
                {year}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <div className={`flex-1 min-w-[150px]`}>
        ...
        >
          ...
          <SelectContent className={isRTL ? 'text-right' : 'text-left'}>
            <SelectItem value="all" className={isRTL ? 'text-right' : 'text-left'}>{t('admin.reports.allMonths')}</SelectItem>
            {months.map((month) => (
              <SelectItem key={month.value} value={month.value.toString()} className={isRTL ? 'text-right' : 'text-left'}>
                {month.label}
              </SelectItem>
            ))}
          </SelectContent>
