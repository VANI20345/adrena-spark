...
/**
 * Fetch admin stats with caching and 30-day comparison
 */
export async function fetchAdminStats() {
  return withCache('admin-stats', async () => {
    ...
    const [
      ...
      supabase.from('profiles').select('*', { count: 'exact', head: true }).lte('created_at', thirtyDaysAgo.toISOString()),
      ...
      supabase.from('bookings').select('platform_commission').eq('status', 'confirmed').lte('created_at', thirtyDaysAgo.toISOString()),
      supabase.from('groups').select('*', { count: 'exact', head: true }),
      supabase.from('reported_messages').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('provider_verifications' as any).select('*', { count: 'exact', head: true }).eq('status', 'verified'),
    ...
    const eventRevenue = (bookingsData.data || []).reduce((sum, b) => sum + (b.platform_commission || 0), 0);
    const serviceRevenue = (serviceBookingsData.data || []).reduce((sum: number, b: any) => sum + (b.platform_commission || 0), 0);
    const totalRevenue = eventRevenue + serviceRevenue;
    const revenue30dAgo = (bookingsData30d.data || []).reduce((sum, b) => sum + (b.platform_commission || 0), 0);
    ...
    const inactiveUsers = inactiveUsersCount.count || 0;
    
    // Calculate percentage changes
    const userGrowthPercent = users30dAgo > 0 ? ((totalUsers - users30dAgo) / users30dAgo) * 100 : 0;
    const eventGrowthPercent = events30dAgo > 0 ? ((totalEvents - events30dAgo) / events30dAgo) * 100 : 0;
    const revenueGrowthPercent = revenue30dAgo > 0 ? ((totalRevenue - revenue30dAgo) / revenue30dAgo) * 100 : 0;

    // Calculate real DAU/MAU and churn
    const dauMau = totalUsers > 0 ? activeUsers / totalUsers : 0;
    const userChurn = totalUsers > 0 ? (inactiveUsers / totalUsers) * 100 : 0;

    return {
      totalUsers,
      userGrowth: userGrowthPercent >= 0 ? `+${Math.abs(userGrowthPercent).toFixed(1)}%` : `-${Math.abs(userGrowthPercent).toFixed(1)}%`,
      userGrowthPercent,
      totalEvents,
      eventGrowth: eventGrowthPercent >= 0 ? `+${Math.abs(eventGrowthPercent).toFixed(1)}%` : `-${Math.abs(eventGrowthPercent).toFixed(1)}%`,
      eventGrowthPercent,
      ...
      bookingConversion: (totalBookings / (totalUsers || 1)) * 100,
      ...
      reportedMessages: reportedMessagesCount.count || 0,
