...
export async function fetchAdminStats() {
  return withCache('admin-stats', async () => {
    ...
    const [
      ...
      supabase.from('bookings').select('total_amount').eq('status', 'confirmed').lte('created_at', thirtyDaysAgo.toISOString()),
    ...
    const revenue30dAgo = (bookingsData30d.data || []).reduce((sum, b) => sum + (b.total_amount || 0), 0);
