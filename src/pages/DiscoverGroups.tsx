...
export default function DiscoverGroups() {
  ...
  const hasActiveFilters = activeFilters && (
    ...
    (activeFilters.participantRange && (activeFilters.participantRange[0] !== 0 || activeFilters.participantRange[1] !== 500)) ||
    (activeFilters.ageRange && (activeFilters.ageRange[0] !== 18 || activeFilters.ageRange[1] !== 65))
  );
  ...
  const { data: discoverGroups = [], isLoading } = useSupabaseQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from('event_groups')
        .select('*, cities!city_id(name, name_ar), image_url')
        .is('archived_at', null);
      ...
      if (activeFilters) {
        ...
        // Apply age range filter (groups whose age range overlaps with filter)
        if (activeFilters.ageRange) {
          // Only filter groups that have age restrictions set
          const minAge = activeFilters.ageRange[0];
          const maxAge = activeFilters.ageRange[1];
          query = query.or(`and(min_age.lte.${maxAge},max_age.gte.${minAge}),and(min_age.is.null,max_age.is.null)`);
        }
      }
