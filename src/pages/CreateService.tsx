...
const formSchema = z.object({
  ...
  pricingType: z.enum(['per_hour', 'per_day', 'fixed']).default('per_hour'),
  imageUrl: z.string().optional(),
  // Availability settings
  availabilityType: z.enum(['full_day', 'specific_hours']).default('specific_hours'),
  availableFrom: z.string().default('08:00'),
  availableTo: z.string().default('22:00'),
  bookingDurationMinutes: z.number().min(15, "مدة الحجز يجب أن تكون 15 دقيقة على الأقل").default(30),
  // Capacity - maximum concurrent bookings for the same time slot
  maxCapacity: z.number().min(1, "السعة القصوى يجب أن تكون 1 على الأقل").default(1),
}).refine(
  (data) => {
    // If specific hours, availableFrom must be before availableTo
    if (data.availabilityType === 'specific_hours') {
      return data.availableFrom < data.availableTo;
    }
    return true;
  },
  { message: "وقت البدء يجب أن يكون قبل وقت الانتهاء", path: ["availableFrom"] }
).refine(
  (data) => {
    // If per_hour pricing, price must be set
    if (data.pricingType === 'per_hour' && data.price <= 0) {
      return false;
    }
    return true;
  },
  { message: "السعر بالساعة مطلوب للتسعير بالساعة", path: ["price"] }
);
...
const CreateService = () => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      imageUrl: "",
      availabilityType: 'specific_hours' as const,
      availableFrom: '08:00',
      availableTo: '22:00',
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const { error } = await supabase.from('services').insert({
        ...
        service_type: data.pricingType === 'per_hour' ? 'hourly' : 'fixed',
      });
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>الفترة الزمنية للحجز (بالدقائق)</FormLabel>
                        <Select 
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          هذا يحدد الفترات الزمنية المتاحة للحجز (مثال: كل 30 دقيقة)
                        </p>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>السعة القصوى (عدد الحجوزات المتزامنة)</FormLabel>
                        <FormControl>
                          <Input 
                            type="number" 
                            min="1"
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          كم عميل يمكنه حجز نفس الوقت؟ (1 = حجز حصري، أكثر = حجوزات متعددة)
                        </p>
                        <FormMessage />
                      </FormItem>
                    )}
