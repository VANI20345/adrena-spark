...
const Messages = () => {
  ...
  const { data: tickets, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const latestPerTicket = new Map<string, { id: string; sender_id: string; created_at: string }>();
      ...
      const mappedTickets = data.map(ticket => {
        ...
        const lastReadMsgId = typeof window !== 'undefined' ? localStorage.getItem(`ticket_read_${ticket.id}`) : null;
        const is_unread = lastMsg && lastMsg.sender_id !== user.id ? lastReadMsgId !== lastMsg.id : false;
  ...
  // Mark selected ticket as read in localStorage
  useEffect(() => {
    if (selectedTicketId && messages && messages.length > 0) {
      const lastMsg = messages[messages.length - 1];
      if (lastMsg) {
        localStorage.setItem(`ticket_read_${selectedTicketId}`, lastMsg.id);
        // Invalidate queries to instantly clear notification badges
        queryClient.invalidateQueries({ queryKey: ['unread-tickets-count'] });
        queryClient.invalidateQueries({ queryKey: ['user-messages-tickets'] });
      }
    }
  }, [selectedTicketId, messages, queryClient]);
          ...
          >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
              <TabsList className="w-full mb-4 h-12 grid grid-cols-2">
                <TabsTrigger value="open" className="text-xs sm:text-sm gap-1.5">
                ...
                <TabsTrigger value="closed" className="text-xs sm:text-sm gap-1.5">
                            ...
                            >
                              ...
                              <div className="flex items-start gap-3">
                                <div className={cn(
                                  ...
                                  <div className={cn(
                                     "absolute -bottom-1 end-[-4px] w-4 h-4 rounded-full flex items-center justify-center text-white",
                          ...
                          return (
                            ...
                            >
                              <div className="relative">
                                ...
                                {isVerifiedReply && (
                                  <div className="absolute -bottom-1 end-[-4px] w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
                               ...
                               <div className={cn(
                                ...
                                isOwn 
                                  ...
                                  : isVerifiedReply
                                    ? "bg-green-100 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-ss-sm"
