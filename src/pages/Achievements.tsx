...
const AchievementsPage = () => {
  ...
  const { data: userStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [bookings, trainings, groups, groupsCreated, gamification] = await Promise.all([
        ...
        supabase.from('user_gamification').select('referral_count, points_balance, is_shield_member').eq('user_id', user.id).single()
      ]);
      ...
      return {
        ...
        referral_count: gamification.data?.referral_count || 0,
        points_balance: gamification.data?.points_balance || 0,
        is_shield_member: gamification.data?.is_shield_member || false
      };
    },
    enabled: !!user?.id
  });
