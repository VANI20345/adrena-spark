...
const Settings = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Load phone from profile_contacts
      loadPhoneFromContacts();
  ...
  const loadPhoneFromContacts = async () => {
    if (!user?.id) return;
    ...
    if (data?.phone) {
      setAccountSettings(prev => ({ ...prev, phone: data.phone || '' }));
    }
  };
                        ...
                        onChange={async (e) => {
                          ...
                          if (user?.id) {
                            try {
                              ...
                              toast.error(isRTL ? 'تعذر حفظ الإعداد' : 'Failed to save setting');
                            }
                          }
                        }}
