...
import { FeatureTogglesTab } from '@/components/SuperAdmin/FeatureTogglesTab';
...
import { PaymentHoldsSection } from '@/components/SuperAdmin/PaymentHoldsSection';
...
import { SystemAlertsTab } from '@/components/SuperAdmin/SystemAlertsTab';
...
const SuperAdminPanel = () => {
  ...
  React.useEffect(() => {
    const handleTabSwitch = (event: CustomEvent) => {
      // Support both event.detail as string (from Quick Actions) or event.detail.tab
      const tabId = typeof event.detail === 'string' ? event.detail : event.detail?.tab;
      if (tabId) {
        setActiveTab(tabId);
      }
    };
    window.addEventListener('switch-super-admin-tab', handleTabSwitch as EventListener);
    return () => {
      window.removeEventListener('switch-super-admin-tab', handleTabSwitch as EventListener);
    };
  }, []);
  ...
  const tabs = [
    ...
    { id: 'payment-holds', icon: Lock, labelAr: 'هولدز الدفع', labelEn: 'Payment Holds' },
    { id: 'withdrawals', icon: Banknote, labelAr: 'طلبات السحب', labelEn: 'Withdrawals' },
    { id: 'refunds', icon: Undo2, labelAr: 'طلبات الاسترداد', labelEn: 'Refunds' },
    { id: 'system-alerts', icon: AlertTriangle, labelAr: 'تنبيهات النظام', labelEn: 'System Alerts' },
    { id: 'feature-toggles', icon: ToggleLeft, labelAr: 'التحكم في الميزات', labelEn: 'Feature Toggles' },
      ...
      <main className="flex-1 container mx-auto px-4 py-8" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <TabsContent value="overview" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "overview" && <SuperAdminOverviewTab />}
          </TabsContent>

          <TabsContent value="reports" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "reports" && <AdminReportsTab />}
          </TabsContent>

          <TabsContent value="staff-dashboard" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "staff-dashboard" && <StaffDashboardTab />}
          </TabsContent>

          <TabsContent value="city-management" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "city-management" && <CityManagementTab />}
          </TabsContent>

          <TabsContent value="social-media" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "social-media" && <SocialMediaManagementTab />}
          </TabsContent>

          <TabsContent value="suspicious-activity" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "suspicious-activity" && <SuspiciousActivityTab />}
          </TabsContent>

          <TabsContent value="event-activation" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "event-activation" && <EventActivationTab />}
          </TabsContent>

          <TabsContent value="role-management" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "role-management" && <RoleManagementTab />}
          </TabsContent>

          <TabsContent value="admin-performance" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "admin-performance" && <AdminPerformanceTab />}
          </TabsContent>

          <TabsContent value="financials" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "financials" && <FinancialDashboardTab />}
          </TabsContent>

          <TabsContent value="payment-holds" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "payment-holds" && <PaymentHoldsSection />}
          </TabsContent>

          <TabsContent value="withdrawals" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "withdrawals" && <WithdrawalManagementTab />}
          </TabsContent>

          <TabsContent value="refunds" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "refunds" && <RefundManagementTab />}
          </TabsContent>

          <TabsContent value="system-alerts" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "system-alerts" && <SystemAlertsTab />}
          </TabsContent>


          <TabsContent value="feature-toggles" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "feature-toggles" && <FeatureTogglesTab />}
          </TabsContent>

          <TabsContent value="contact-settings" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "contact-settings" && <ContactSettingsTab />}
          </TabsContent>

          <TabsContent value="commission-settings" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "commission-settings" && <CommissionSettingsTab isRTL={isRTL} />}
          </TabsContent>

          {/* System Tab - Super Admin exclusive */}
          <TabsContent value="system" className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "system" && (
          ...
          <TabsContent value="notifications" className="space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "notifications" && (
              <Card>
                <CardHeader dir={isRTL ? 'rtl' : 'ltr'} className={isRTL ? 'text-right' : 'text-left'}>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <Button onClick={() => setNotificationDialogOpen(true)} className="self-start">
                    ...
                    <Card>
                      <CardContent className="pt-6">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <p className="text-2xl font-bold">{sharedTotalUsers}</p>
                        </div>
                      </CardContent>
                    </Card>
                    <Card>
          ...
          <TabsContent value="activity-logs" dir={isRTL ? 'rtl' : 'ltr'}>
            {activeTab === "activity-logs" && <SuperAdminActivityLogsTab />}
          </TabsContent>
        </Tabs>
      </main>
