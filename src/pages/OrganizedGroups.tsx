...
export default function OrganizedGroups() {
  ...
  const { data: organizerGroups = [], isLoading } = useSupabaseQuery({
    queryKey: ['all-organizer-groups', user?.id, searchTerm],
    queryFn: async () => {
      if (!user) return [];
  ...
  const { data: groupMembersMapData } = useSupabaseQuery({
    queryKey: ['group-members-avatars-organized'],
    queryFn: async () => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
      <Navbar />
      <main className="container mx-auto px-4 py-8 flex-grow flex-1">
        <div className="mb-8">
          <Button
            variant="ghost"
          ...
          <p className="text-muted-foreground">
            {isRTL 
              ? `لديك ${organizerGroups?.length || 0} مجموعة تديرها` 
              : `You organize ${organizerGroups?.length || 0} groups`}
          </p>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-12 text-muted-foreground">
              {searchTerm 
                ...
                : (isRTL ? 'لم تنشئ أي مجموعة بعد' : 'You haven\'t created any groups yet')}
            </div>
          )}
        </div>
      </main>
