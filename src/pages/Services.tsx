...
const Services = () => {
  ...
  // Fetch ALL approved services reliably - no filtering at query level to prevent missing items
  const { data: services = [], isLoading: servicesLoading, error: servicesError } = useQuery({
    queryKey: ['services', 'all-approved'],
    queryFn: async () => {
      // Single comprehensive query - fetch all approved services
      ...
      console.log(`[Services] Fetched ${data?.length || 0} approved services`);
      
      return data || [];
    },
    staleTime: 2 * 60 * 1000, // Refresh more frequently to catch new services
    gcTime: 5 * 60 * 1000,
  ...
  // IMPORTANT: Use explicit type checks to prevent items from being lost
  const servicesSection = filteredServices.filter(s => {
    const type = s.service_type?.toLowerCase()?.trim();
    // Include if no type, type is 'other', or type is empty/null/undefined
    return !type || type === 'other' || type === '';
  });
  
  const trainingsSection = filteredServices.filter(s => {
    const type = s.service_type?.toLowerCase()?.trim();
  ...
  const discountsSection = filteredServices.filter(s => {
    const type = s.service_type?.toLowerCase()?.trim();
    // Include if type is 'discount' OR has a discount percentage (regardless of type)
  ...
  const allServicesDisabled = toggles.services === false && toggles.trainings === false && toggles.discounts === false;

  if (allServicesDisabled) {
    return <FeatureDisabled featureKey="services" />;
  }
