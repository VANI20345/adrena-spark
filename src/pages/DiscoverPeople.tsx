...
const DiscoverPeople = () => {
  ...
  const getSuggestionLabel = (reason: string | null | undefined) => {
    if (!reason) return null;
    switch (reason) {
      case 'mutual_group':
        return {
          icon: Users2,
          text: isRTL ? 'من مجموعاتك' : 'From your groups',
          color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
        };
      case 'mutual_connection':
        return {
          icon: UserCheck,
          text: isRTL ? 'متابعون مشتركون' : 'Mutual followers',
          color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300'
        };
      case 'popular':
        return {
          icon: TrendingUp,
          text: isRTL ? 'شائع' : 'Popular',
  ...
  // Render user card with suggestion reason and shared groups
  const renderUserCard = (user: any, index: number) => {
    const suggestionInfo = getSuggestionLabel(user.suggestion_reason);
    ...
    return (
      ...
      >
        {suggestionInfo && (
          <div className={cn("absolute -top-2 z-10", isRTL ? "right-2" : "left-2")}>
            <Badge className={cn("gap-1 text-xs px-2 py-0.5", suggestionInfo.color, isRTL && "flex-row-reverse")}>
