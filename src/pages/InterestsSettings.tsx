            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {interests.map((interest) => (
                <div key={interest.id} className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <Checkbox
                    id={interest.id}
                    checked={selectedInterests.includes(interest.name_ar) || selectedInterests.includes(interest.name)}
                    onCheckedChange={() => handleInterestToggle(language === 'ar' ? interest.name_ar : interest.name)}
                  />
                  <Label 
                    htmlFor={interest.id} 
