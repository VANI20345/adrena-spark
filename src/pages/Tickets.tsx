...
const Tickets = () => {
  ...
  const formatDate = (date: string) => {
    return toEnglishDigits(format(new Date(date), 'PPP', { locale: isRTL ? ar : undefined }));
  };

  const formatTime = (date: string) => {
    return toEnglishDigits(format(new Date(date), 'p', { locale: isRTL ? ar : undefined }));
  };

  const [viewTicket, setViewTicket] = useState<TicketData | null>(null);

  const shareTicket = async (ticket: TicketData) => {
    const title = (isRTL ? ticket.events?.title_ar : ticket.events?.title) || 'Ticket';
    const shareText = isRTL
      ? `تذكرتي لفعالية ${title}\nرقم الحجز: ${ticket.booking_reference}`
      : `My ticket for ${title}\nBooking Ref: ${ticket.booking_reference}`;
    const shareUrl = `${window.location.origin}/tickets`;
    try {
      if (navigator.share) {
      ...
      toast.success(isRTL ? 'تم نسخ تفاصيل التذكرة' : 'Ticket details copied to clipboard');
    } catch (err: any) {
      if (err?.name === 'AbortError') return;
      toast.error(isRTL ? 'تعذّرت المشاركة على هذا الجهاز' : 'Sharing is not available on this device');
    }
  };
  ...
  const TicketCard = ({ ticket }: { ticket: TicketData }) => (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow">
      <CardContent className="p-0">
        <div className="grid md:grid-cols-3 gap-0" dir={isRTL ? 'rtl' : 'ltr'}>
          {/* Event Image */}
          ...
          <div className="p-6 flex flex-col justify-between">
            ...
            <div className="flex gap-2 mt-4">
              <Button size="sm" variant="outline" className="flex-1" onClick={() => setViewTicket(ticket)}>
                <Ticket className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />
                {isRTL ? 'عرض التذكرة' : 'View Ticket'}
              </Button>
              <Button size="sm" variant="outline" className="flex-1" onClick={() => shareTicket(ticket)}>
          ...
          {/* QR Code */}
          <div className="border-t md:border-t-0 md:border-s border-dashed bg-muted/30 p-6 flex flex-col items-center justify-center">
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <Dialog open={!!viewTicket} onOpenChange={(open) => !open && setViewTicket(null)}>
        <DialogContent className="max-w-md" dir={isRTL ? 'rtl' : 'ltr'}>
          <DialogHeader>
            <DialogTitle>{isRTL ? 'تفاصيل التذكرة' : 'Ticket Details'}</DialogTitle>
            <DialogDescription>
              {viewTicket ? (isRTL ? viewTicket.events?.title_ar : viewTicket.events?.title) : ''}
            </DialogDescription>
          </DialogHeader>
          {viewTicket && (
            <div className="space-y-4">
              <div className="mx-auto w-52 h-52 bg-white rounded-xl p-3 border-2 border-dashed border-primary/30 flex items-center justify-center">
                <QRCodeSVG value={viewTicket.booking_reference} size={180} level="H" />
              </div>
              <div className="text-center">
                <p className="text-xs text-muted-foreground">{isRTL ? 'رقم الحجز' : 'Booking Reference'}</p>
                <p className="font-mono font-bold text-primary text-lg">{viewTicket.booking_reference}</p>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                {viewTicket.events?.start_date && (
                  <div>
                    <p className="text-muted-foreground text-xs">{isRTL ? 'التاريخ' : 'Date'}</p>
                    <p className="font-medium">{formatDate(viewTicket.events.start_date)}</p>
                  </div>
                )}
                {viewTicket.events?.start_date && (
                  <div>
                    <p className="text-muted-foreground text-xs">{isRTL ? 'الوقت' : 'Time'}</p>
                    <p className="font-medium">{formatTime(viewTicket.events.start_date)}</p>
                  </div>
                )}
                {viewTicket.events?.location && (
                  <div className="col-span-2">
                    <p className="text-muted-foreground text-xs">{isRTL ? 'الموقع' : 'Location'}</p>
                    <p className="font-medium">{isRTL ? viewTicket.events?.location_ar : viewTicket.events?.location}</p>
                  </div>
                )}
                <div>
                  <p className="text-muted-foreground text-xs">{isRTL ? 'عدد التذاكر' : 'Tickets'}</p>
                  <p className="font-medium">{viewTicket.quantity}</p>
                </div>
              </div>
              <Button className="w-full" onClick={() => shareTicket(viewTicket)}>
                <Share2 className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />
                {isRTL ? 'مشاركة التذكرة' : 'Share Ticket'}
              </Button>
            </div>
          )}
        </DialogContent>
