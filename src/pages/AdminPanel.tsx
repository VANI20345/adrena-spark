...
const AdminPanel = () => {
  ...
  const { data: users = [], isLoading: usersLoading, refetch: refetchUsers } = useOptimizedQuery(
    ...
    { enabled: (userRole === 'admin' || userRole === 'super_admin') && activeTab === 'users', staleTime: 2 * 60 * 1000 }
  );

  // Use shared hook for consistent total users count across admin and super-admin
  const { data: sharedTotalUsers = 0 } = useTotalUsersCount();
  const totalUsersCount = sharedTotalUsers;
  ...
  const { data: pendingProviders = [], isLoading: providersLoading, refetch: refetchProviders } = useOptimizedQuery(
    ...
    { enabled: (userRole === 'admin' || userRole === 'super_admin') && activeTab === 'provider-verification', staleTime: 2 * 60 * 1000 }
  );
      ...
      <main className={`flex-1 container mx-auto px-4 py-8 ${isRTL ? 'rtl' : 'ltr'}`}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="provider-verification">
            {activeTab === 'provider-verification' && (
          ...
          <TabsContent value="suspended">
            {activeTab === 'suspended' && <SuspendedUsersTab />}
          </TabsContent>
          ...
          <TabsContent value="groups-management">
            {activeTab === 'groups-management' && <GroupManagementTab />}
          </TabsContent>

          {/* Unified Tickets Tab */}
          <TabsContent value="tickets">
            {activeTab === 'tickets' && <UnifiedTicketsTab />}
          </TabsContent>
          ...
          <TabsContent value="entity-reports">
            {activeTab === 'entity-reports' && <EntityReportsTab />}
          </TabsContent>
