...
const EventDetails = () => {
  ...
  const formatDateRange = (startDate: string, endDate: string) => {
    ...
    if (start.toDateString() === end.toDateString()) {
      return toEnglishDigits(format(start, 'dd MMMM yyyy', { locale }));
    }

    return `${toEnglishDigits(format(start, 'dd', { locale }))} - ${toEnglishDigits(format(end, 'dd MMMM yyyy', { locale }))}`;
  };
            ...
            <div>
              <div className="flex flex-wrap gap-3 mb-4">
                {(() => {
                  ...
                  if (start <= now) {
                    return <Badge className="bg-amber-500 text-white hover:bg-amber-600">{isRTL ? 'جارية الآن' : 'Ongoing'}</Badge>;
                  }
                  if (hoursToStart <= 24) {
                    return <Badge className="bg-amber-500 text-white hover:bg-amber-600">{isRTL ? `تبدأ خلال ${Math.max(1, Math.ceil(hoursToStart))} ساعة` : `Starts in ${Math.max(1, Math.ceil(hoursToStart))}h`}</Badge>;
                  }
                  return null;
                })()}
              ...
              <div className="flex flex-wrap gap-6 text-muted-foreground mt-4">
                ...
                {schedules.length === 1 && (
                  <div className="flex items-center gap-2">
                    ...
                    {toEnglishDigits(format(new Date(`2000-01-01 ${schedules[0].start_time}`), 'p', { locale: isRTL ? ar : undefined }))} - {toEnglishDigits(format(new Date(`2000-01-01 ${schedules[0].end_time}`), 'p', { locale: isRTL ? ar : undefined }))}
                  </div>
                )}
                <div className="flex items-center gap-2">
                      ...
                      <TabsContent key={idx} value={idx.toString()} className="space-y-4 mt-4">
                        <div className="space-y-3">
                          <div className="flex items-center gap-2 text-lg font-semibold">
                            ...
                            {toEnglishDigits(format(new Date(schedule.schedule_date), 'PPP', { locale: isRTL ? ar : undefined }))}
                          </div>
                          <div className="flex items-center gap-2">
                            <Clock className="w-5 h-5 text-primary" />
                            <span>
                              {toEnglishDigits(format(new Date(`2000-01-01 ${schedule.start_time}`), 'p', { locale: isRTL ? ar : undefined }))} - 
                              {toEnglishDigits(format(new Date(`2000-01-01 ${schedule.end_time}`), 'p', { locale: isRTL ? ar : undefined }))}
                            </span>
                          </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="outline" onClick={handleContact} className="gap-2">
                    ...
                    {isRTL ? 'تواصل مع المنظم' : 'Contact Organizer'}
                  </Button>
                </div>
              </CardContent>
            </Card>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <span className="text-xs text-muted-foreground mt-2 inline-block">
                            {toEnglishDigits(format(new Date(review.created_at), 'PP', { locale: isRTL ? ar : undefined }))}
                          </span>
                        </div>
                      </div>
            ...
            <Card className="sticky top-24">
              ...
              <CardContent className="space-y-4">
                ...
                {(() => {
                  ...
                  const start = new Date(event.start_date);
                  const end = new Date(event.end_date);
                  const hoursToStart = Math.max(1, Math.ceil((start.getTime() - now.getTime()) / 3600000));
                  const started = start <= now;
                  const ended = end < now;
                  const withinDay = !started && !ended && hoursToStart <= 24;
                  const isFull = event.current_attendees >= event.max_attendees;
                  const disabled = event.status === 'pending' || event.status === 'cancelled' || started || ended || withinDay || isFull;
                  const label = event.status === 'pending' ? (isRTL ? 'قيد المراجعة' : 'Pending Review')
                    : event.status === 'cancelled' ? (isRTL ? 'ملغية' : 'Cancelled')
                    : ended ? (isRTL ? 'انتهت' : 'Ended')
                    : started ? (isRTL ? 'جارية الآن' : 'Ongoing')
                    : withinDay ? (isRTL ? `تبدأ خلال ${hoursToStart} ساعة` : `Starts in ${hoursToStart}h`)
                    : isFull ? (isRTL ? 'مكتملة' : 'Full')
