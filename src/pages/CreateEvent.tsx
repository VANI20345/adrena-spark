  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({
        ...
        description: isRTL ? 'تم إنشاء الفعالية ورفعها للأدمن بانتظار الموافقة' : 'Event created and submitted to admin for approval',
      });
            ...
            <Card>
              <CardContent className="pt-6 space-y-6">
                ...
                >
                  ...
                  {schedules.length > 0
                    ? (schedules.length === 1
                        ? (isRTL ? 'يوم واحد محدد — انقر للتعديل' : '1 day set — click to edit')
                        : (isRTL ? `${schedules.length} أيام محددة — انقر للتعديل` : `${schedules.length} days set — click to edit`))
                    : isRTL
                    ? 'تحديد التاريخ والوقت (يوم واحد أو أكثر)'
                    : 'Set Date & Time (single or multi-day)'}
                </Button>
                <p className="text-xs text-muted-foreground -mt-1">
                  {isRTL
                    ? 'لإنشاء فعالية تمتد على أكثر من يوم، أضف كل يوم على حدة داخل النافذة مع تحديد وقت البداية والنهاية لكل يوم.'
                    : 'To create a multi-day event, add each day inside the dialog with its own start and end time.'}
                </p>
                  ...
                  <div className="space-y-2">
                    ...
                    {schedules.map((schedule, index) => (
                      <div key={index} className="text-sm text-muted-foreground bg-muted p-3 rounded-md">
                        {toEnglishDigits(format(schedule.date, 'PPP', { locale: isRTL ? ar : undefined }))} -{' '}
