...
const CheckoutSuccess = () => {
  ...
  // Support both state (from PaymentGateway) and URL params (from 3DS redirect)
  const bookingId = location.state?.bookingId || searchParams.get('booking_id');
  ...
  const from3DS = searchParams.get('from_3ds') === 'true';
  ...
  // Auto-verify payment if coming from 3DS and booking is still pending
  useEffect(() => {
    if (from3DS && booking && booking.status === 'pending_payment' && paymentId) {
      verifyPayment();
    }
  }, [from3DS, booking, paymentId]);
  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      if (data.status === 'confirmed' || data.booking?.status === 'confirmed') {
        // Update local booking state immediately
        setBooking(prev => prev ? { ...prev, status: 'confirmed' } : prev);
        
        // Also refresh from database to get all updated fields
        ...
        toast({
          ...
          description: isRTL ? 'لم نتمكن من تأكيد الدفع' : 'We could not confirm your payment',
          variant: 'destructive'
        });
      } else if (data.status === 'pending') {
        toast({
          title: isRTL ? 'الدفع قيد المعالجة' : 'Payment Processing',
          description: isRTL ? 'جاري معالجة الدفع' : 'Your payment is being processed'
        });
      }
    } catch (error) {
