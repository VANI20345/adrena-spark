...
export default function Leaderboard() {
  ...
  const renderLeaderboardEntry = (entry: any, index: number) => {
    ...
    return (
      <Card 
        key={entry.id}
        className={`${isCurrentUser ? 'border-primary shadow-lg' : ''} ${isTopThree ? 'bg-gradient-to-r from-primary/5 to-transparent' : ''} cursor-pointer hover:shadow-md transition-all`}
      ...
      >
        <CardContent className="p-4">
          <div className="flex items-center gap-4">
            ...
            <Link to={`/user/${entry.user_id}`} onClick={(e) => e.stopPropagation()}>
              <Avatar className="h-12 w-12 ring-2 ring-background hover:ring-primary/30 transition-all cursor-pointer">
