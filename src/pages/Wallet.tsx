...
import InvoicesSection from '@/components/Wallet/InvoicesSection';
...
const WalletPage = () => {
  ...
  // Phase 8.1 — Provider wallet summary (available / held / pending) + holds
  const { data: walletSummary } = useSupabaseQuery({
    queryKey: ['provider_wallet_summary', user?.id],
    queryFn: useCallback(async () => {
      if (!user?.id) return null;
      const { data, error } = await supabase.rpc('get_provider_wallet_summary');
      if (error) throw error;
      return data as any;
    }, [user?.id]),
    enabled: !!user?.id,
  ...
  const heldBalance = Number(walletSummary?.held_balance || 0);
  const pendingWithdrawAmount = Number(walletSummary?.pending_withdraw || pendingAmount || 0);
  const holdsList: any[] = Array.isArray(walletSummary?.holds) ? walletSummary!.holds : [];
  ...
  const handleWithdraw = async () => {
    ...
    // Source of truth = backend get_provider_available_balance / wallet summary.
    // We compute UI cap from walletSummary (allowing full balance withdrawal)
    const summaryBalance = Number(walletSummary?.balance ?? walletData?.balance ?? 0);
    const availableBalance = Math.max(0, summaryBalance);
  ...
  const currentBalance = Number(walletSummary?.balance ?? walletData?.balance ?? 0);
  const totalEarnings = walletData?.total_earned || 0;
  const totalWithdrawn = walletData?.total_withdrawn || 0;
  const heldForUI = Number(walletSummary?.held_balance ?? 0);
  const pendingForUI = Number(walletSummary?.pending_withdraw ?? pendingAmount ?? 0);
    ...
    <div className="min-h-screen bg-background" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        >
                  ...
                  <DialogContent className="sm:max-w-md" dir={isRTL ? 'rtl' : 'ltr'}>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 rounded-xl bg-success/10 border border-success/20">
                        <div className="text-sm text-muted-foreground mb-1">{t('wallet.currentBalance')}</div>
          ...
          {((userRole as string) === 'provider' || (userRole as string) === 'service_provider' || heldBalance > 0 || pendingWithdrawAmount > 0) && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
                ...
                <CardContent className="p-6">
                  <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <p className="text-sm text-muted-foreground">{isRTL ? 'متاح للسحب' : 'Available'}</p>
                      <p className="text-2xl font-bold tabular-nums" dir="ltr">
                        {currentBalance.toLocaleString()} <span className="text-sm font-normal">SAR</span>
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-background">
                <CardContent className="p-6">
                  <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <p className="text-sm text-muted-foreground">{isRTL ? 'محجوز (في انتظار قرار الأدمن)' : 'Held (Pending Admin)'}</p>
                      <p className="text-2xl font-bold tabular-nums text-amber-600" dir="ltr">
                        {heldBalance.toLocaleString()} <span className="text-sm font-normal">SAR</span>
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="border-destructive/20 bg-gradient-to-br from-destructive/5 to-background">
                <CardContent className="p-6">
                  <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <p className="text-sm text-muted-foreground">{isRTL ? 'قيد السحب' : 'Pending Withdraw'}</p>
                      <p className="text-2xl font-bold tabular-nums text-destructive" dir="ltr">
                        {pendingWithdrawAmount.toLocaleString()} <span className="text-sm font-normal">SAR</span>
                      </p>
                    </div>
                  </div>
                </CardContent>
          ...
          {holdsList.length > 0 && (
            ...
            >
              <Card className="border-amber-500/20">
                <CardContent className="p-6">
                  <div className={`flex items-center gap-2 mb-4 ${isRTL ? 'flex-row-reverse text-right' : ''}`}>
                    <Clock className="h-5 w-5 text-amber-600" />
                    <h3 className="text-lg font-semibold">
                      {isRTL ? 'تفاصيل الأموال المحجوزة' : 'Held Funds Detail'}
                    </h3>
                  </div>
                  <div className="space-y-2">
                    {holdsList.map((h: any) => {
                      const isDispute = h.complaint_extension || h.review_state === 'dispute_hold';
                      const isReady = h.review_state === 'ready_for_release' && !h.complaint_extension;
                      const releaseDate = new Date(h.hold_until).toLocaleString(isRTL ? 'ar-SA' : 'en-US');
                      let statusLabel: string;
                      let statusColor: string;
                      if (isDispute) {
                        statusLabel = isRTL ? '⚠️ يوجد نزاع — معلّق' : '⚠️ Dispute — On Hold';
                        statusColor = 'text-destructive';
                      } else if (isReady) {
                        statusLabel = isRTL ? '✓ جاهز للمراجعة من الأدمن' : '✓ Ready for admin review';
                        statusColor = 'text-success';
                      } else {
                        statusLabel = isRTL ? `⏳ سيكون قابلاً للمراجعة في ${releaseDate}` : `⏳ Available for review on ${releaseDate}`;
                        statusColor = 'text-amber-600';
                      }
                      return (
                        <div key={h.id} className={`flex items-center justify-between gap-4 p-3 rounded-lg bg-muted/40 ${isRTL ? 'flex-row-reverse' : ''}`}>
                          <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                            <p className="font-mono tabular-nums font-semibold text-amber-600" dir="ltr">
                              {Number(h.held_amount || 0).toFixed(2)} SAR
                            </p>
