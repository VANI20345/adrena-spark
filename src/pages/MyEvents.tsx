...
const MyEventsPage = () => {
  ...
  const handleCancelBooking = async (event: any) => {
    ...
    try {
      // Free / unpaid bookings — direct cancel with no refund flow
      const isFree = !Number(event.totalPaid) || Number(event.totalPaid) === 0;
      if (!isFree) {
      ...
      toast({
        ...
        description: isFree
          ? (isRTL ? 'تم إلغاء الحجز بنجاح' : 'Your booking has been cancelled')
          : (isRTL ? 'تم إلغاء الحجز وسيتم معالجة الاسترداد وفقاً لسياسة الاسترجاع' : 'Your booking has been cancelled and the refund will be processed per the refund policy')
      });
      ...
      if (raw.includes('24') || raw.includes('too late') || raw.includes('cannot cancel') || raw.includes('غير مسموح')) {
        friendly = isRTL
  ...
  const EventCard = ({ event, showActions = true }: { event: any; showActions?: boolean }) => {
    ...
    return (
      <Card className={`overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group ${event.status === 'pending_payment' ? 'border-amber-300 dark:border-amber-700' : ''}`} onClick={() => navigate(`/event/${event.eventId}`)}>
