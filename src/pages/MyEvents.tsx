...
import { Calendar, MapPin, Users, Clock, Star, CalendarDays, ChevronRight, Ticket, Timer, AlertTriangle, CreditCard, RefreshCw } from 'lucide-react';
...
import RateEventDialog from '@/components/Events/RateEventDialog';
...
const MyEventsPage = () => {
  ...
  const [ratingEvent, setRatingEvent] = useState<any | null>(null);
  ...
  useEffect(() => {
    const fetchMyEvents = async () => {
      ...
      try {
        ...
        // Calculate days for multi-day events (calendar-day based, inclusive)
        const calculateDays = (start: string, end: string) => {
          const startDate = new Date(start);
          const endDate = new Date(end);
          // Normalize to midnight local time so partial-day durations don't inflate the count
        ...
        const mapBooking = (b: any, status: string) => {
          ...
          return {
            ...
            formattedDate: toEnglishDigits(format(new Date(b.events.start_date), 'PPP', { locale: dateLocale })),
            formattedTime: toEnglishDigits(format(new Date(b.events.start_date), 'p', { locale: dateLocale })),
        ...
        // Load existing reviews for completed bookings so users can see if they've already rated
        ...
        if (completedIds.length > 0) {
          ...
          if (existingReviews && existingReviews.length > 0) {
            const byBooking = new Map(existingReviews.map(r => [r.booking_id, r.rating]));
            setCompletedEvents(prev => prev.map(ev => byBooking.has(ev.id)
              ? { ...ev, reviewed: true, rating: byBooking.get(ev.id) || 0 }
              : ev
            ));
          }
        }
  ...
  const handleCancelBooking = async (event: any) => {
    // Distinguish unpaid (pending_payment) vs paid bookings by STATUS, not totalPaid.
    const isUnpaid = event.status === 'pending_payment' || event.status === 'pending';

    const confirmMsg = isUnpaid
      ? (isRTL
          ? 'سيُلغى الحجز ويُحرَّر المقعد فوراً — لا يوجد مبلغ مدفوع لاسترداده. هل تريد المتابعة؟'
          : 'This will cancel the booking and free the seat immediately. There is no payment to refund. Continue?')
      : (isRTL
          ? 'سيتم إنشاء طلب استرداد وفق سياسة الاسترجاع. هل أنت متأكد من إلغاء هذا الحجز؟'
          : 'A refund request will be created per the refund policy. Are you sure you want to cancel this booking?');
    ...
    try {
      if (isUnpaid) {
        // Direct cancel — no refund flow, no money moved.
        ...
        // Release the reserved seat (best-effort; ignore error).
        try {
          await supabase.rpc('increment_event_attendees', {
            event_id: event.eventId,
            increment_by: -(Number(event.quantity) || 1),
          });
        } catch (seatErr) {
          console.warn('Seat release failed (non-blocking):', seatErr);
        }
      } else {
        // Paid booking — must go through refund RPC.
        ...
        if (error) {
          console.error('request_refund RPC error:', error);
          throw error;
        }
      ...
      toast({
        ...
        description: isUnpaid
          ? (isRTL ? 'تم إلغاء الحجز وتحرير المقعد.' : 'Booking cancelled and seat released.')
          : (isRTL
              ? 'تم إنشاء طلب الاسترداد وسيتم معالجته وفق السياسة.'
              : 'A refund request has been created and will be processed per the policy.'),
      });
      setRefreshTrigger(prev => prev + 1);
    } catch (error: any) {
      console.error('Cancel booking failed:', error);
      const raw = String(error?.message || '').toLowerCase();
      let friendly = error?.message || (isRTL ? 'حدث خطأ غير متوقع' : 'An unexpected error occurred');

      if (raw.includes('function') && raw.includes('request_refund') && (raw.includes('does not exist') || raw.includes('not found') || raw.includes('schema cache'))) {
        friendly = isRTL
          ? 'خدمة الاسترداد غير متاحة حالياً. يرجى التواصل مع الدعم لإلغاء الحجز المدفوع.'
          : 'The refund service is not available yet. Please contact support to cancel a paid booking.';
      } else if (raw.includes('24') || raw.includes('too late') || raw.includes('cannot cancel') || raw.includes('غير مسموح')) {
        friendly = isRTL
          ? 'لا يمكن الإلغاء قبل أقل من 24 ساعة من بدء الفعالية.'
          : 'Bookings cannot be cancelled within 24 hours of the event start time.';
      }

      toast({
        title: isRTL ? 'تعذّر إلغاء الحجز' : 'Cancellation Failed',
  ...
  const renderTimeRemaining = (event: any) => {
    if (event.status === 'pending_payment') return null;

    // Ongoing / Starts-in badge (within 24h of start OR during event)
    if (event.startDate) {
      const now = new Date();
      const start = new Date(event.startDate);
      const end = new Date(event.endDate || event.startDate);
      const hoursToStart = (start.getTime() - now.getTime()) / 3600000;
      if (start <= now && end >= now) {
        return (
          <Badge className="bg-amber-500 text-white hover:bg-amber-600">
            <Timer className="h-3 w-3 me-1" />
            {isRTL ? 'جارية الآن' : 'Ongoing'}
          </Badge>
        );
      }
      if (hoursToStart > 0 && hoursToStart <= 24) {
        const h = Math.max(1, Math.ceil(hoursToStart));
        return (
          <Badge className="bg-amber-500 text-white hover:bg-amber-600">
            <Timer className="h-3 w-3 me-1" />
            {isRTL ? `تبدأ خلال ${h} ساعة` : `Starts in ${h}h`}
          </Badge>
        );
      }
    }

    if (!event.daysRemaining) return null;
  ...
  const EventCard = ({ event, showActions = true }: { event: any; showActions?: boolean }) => {
    ...
    return (
      <Card
        className={`overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group ${event.status === 'pending_payment' ? 'border-amber-300 dark:border-amber-700' : ''}`}
        onClick={() => {
          if (event.status === 'pending_payment') {
            if (event.reservationRemaining?.expired) {
              toast({
                title: isRTL ? 'انتهت مهلة الحجز' : 'Reservation Expired',
                description: isRTL ? 'انتهت مهلة 15 دقيقة لإكمال الدفع. الرجاء إعادة الحجز.' : 'The 15-minute payment window has expired. Please book again.',
                variant: 'destructive',
              });
              return;
            }
            navigate('/checkout', {
              state: {
                ...
                eventPrice: event.totalPaid ?? event.price,
                bookingId: event.id,
              },
            });
            return;
            ...
            {showActions && (
              <div className="flex flex-wrap gap-2 pt-4" onClick={(e) => e.stopPropagation()}>
                {event.status === 'unregistered' ? (
                  ...
                  <>
                    ...
                    <Button
                      ...
                      onClick={(e) => { e.stopPropagation(); handleCancelBooking(event); }}
                    >
                      {cancellingId === event.id ? (isRTL ? 'جارٍ الإلغاء...' : 'Cancelling...') : (isRTL ? 'إلغاء الحجز' : 'Cancel')}
                    </Button>
                  </>
                ) : event.status === 'completed' && !event.reviewed ? (
                  <Button size="sm" className="gap-2" onClick={(e) => { e.stopPropagation(); setRatingEvent(event); }}>
                ...
                <Button
                  ...
                  dir={isRTL ? 'rtl' : 'ltr'}
                  onClick={(e) => { e.stopPropagation(); navigate(`/event/${event.eventId}`); }}
                >
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <RateEventDialog
        open={!!ratingEvent}
        onOpenChange={(o) => { if (!o) setRatingEvent(null); }}
        eventId={ratingEvent?.eventId}
        bookingId={ratingEvent?.id}
        eventTitle={ratingEvent?.title}
        onSubmitted={() => { setRatingEvent(null); setRefreshTrigger(p => p + 1); }}
      />
      <Footer />
    </div>
  );
