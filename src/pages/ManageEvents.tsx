...
import { Calendar, MapPin, Users, Eye, Edit, Trash2, Plus, Search, QrCode, MessageCircle, ClipboardCheck } from 'lucide-react';
        ...
        <div className="md:w-3/4 p-6">
          ...
          <CardContent className="p-0">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <div className="text-center p-3 bg-secondary rounded-lg">
                <div className="text-2xl font-bold text-success">{event.revenue?.toLocaleString() || 0}</div>
              ...
              {event.status !== 'completed' && new Date(event.start_date) > new Date() && (
                <>
                  ...
                  <Button size="sm" variant="default" asChild className="gap-2">
                    <Link to={`/event/${event.id}/attendance`}>
                      <ClipboardCheck className="h-4 w-4" />
                      {language === 'ar' ? 'إدارة الحضور' : 'Manage Attendance'}
                    </Link>
                  </Button>
                  <Button size="sm" variant="outline" asChild className="gap-2">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div className="text-success">{t('wallet.riyal')}</div>
              </CardHeader>
              <CardContent>
