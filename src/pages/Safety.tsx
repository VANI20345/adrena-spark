  ...
  return (
    <div className="min-h-screen flex flex-col bg-background" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <main className={`safety-container flex-1 container mx-auto px-4 py-8 ${isRTL ? 'text-right' : 'text-left'}`}>
        ...
        {/* Risk Levels */}
        <Card className="mb-8" dir={isRTL ? 'rtl' : 'ltr'}>
          <CardHeader className={isRTL ? 'text-right' : 'text-left'}>
            <CardTitle className={`flex items-center gap-2 w-full ${isRTL ? 'flex-row-reverse justify-end text-right' : ''}`}>
              <AlertTriangle className="w-5 h-5" />
              <span>{language === 'ar' ? 'مستويات المخاطر' : 'Risk Levels'}</span>
            </CardTitle>
            <CardDescription className={`w-full ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {riskLevels.map((risk, index) => (
                <div key={index} dir={isRTL ? 'rtl' : 'ltr'} className={`p-4 border rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
                  <Badge className={`${risk.color} mb-2`}>{risk.level}</Badge>
                  <p className={`text-sm text-muted-foreground w-full ${isRTL ? 'text-right' : 'text-left'}`}>{risk.description}</p>
                </div>
              ))}
            </div>
          </CardContent>
                ...
                <ul className={`space-y-3 text-sm list-none ${isRTL ? 'pr-0 text-right' : 'pl-0 text-left'}`}>
                  {additionalTips.preparation.items.map((item, index) => (
                    ...
                    >
                      ...
                      <span className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
