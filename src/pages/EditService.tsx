...
import WeeklyScheduleEditor, { WeeklySchedule } from "@/components/Services/WeeklyScheduleEditor";
import TrainingAvailabilitySelector from "@/components/Services/TrainingAvailabilitySelector";
...
const EditService = () => {
  ...
  const [availableForever, setAvailableForever] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (service && user) {
      ...
      form.reset({
        ...
        location_ar: service.location_ar || "",
        is_free: service.is_free || false,
        original_price: service.original_price || 0,
        discount_percentage: service.discount_percentage || 0,
        price: service.price || 0,
        trainer_name: service.trainer_name || "",
        training_level: service.training_level || "",
        start_date: service.start_date || "",
        end_date: service.end_date || "",
        max_capacity: service.max_capacity || 1,
        duration_per_set: service.duration_per_set?.toString() || "",
        number_of_sets: service.number_of_sets || 1,
        city_id: service.city_id || "",
        provided_services: service.provided_services || [],
      });
      ...
      if (service.service_type === 'training') {
        setAvailableFrom(service.available_from || new Date().toISOString().split('T')[0]);
        setAvailableTo(service.available_to || null);
        setAvailableForever(service.available_forever || false);
        
        // Parse weekly_schedule from database
        if (service.weekly_schedule) {
          const schedule = service.weekly_schedule as unknown as WeeklySchedule;
          setWeeklySchedule({
  ...
  }, [service, user, form, language, navigate, toast]);
  ...
  const hasScheduledSlots = Object.values(weeklySchedule).some(slots => slots.length > 0);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (service?.service_type === 'training') {
      if (!hasScheduledSlots) {
        toast({
          title: language === 'ar' ? 'خطأ' : 'Error',
          description: language === 'ar' ? 'يجب إضافة موعد واحد على الأقل في الجدول الأسبوعي' : 'At least one weekly time slot is required',
          variant: 'destructive',
        });
        return;
      }
      ...
      if (!availableFrom) {
        toast({
          title: language === 'ar' ? 'خطأ' : 'Error',
          description: language === 'ar' ? 'يجب تحديد تاريخ البدء' : 'Start date is required',
          variant: 'destructive',
        });
        return;
      }
      ...
      if (!availableForever && !availableTo) {
        toast({
          title: language === 'ar' ? 'خطأ' : 'Error',
          description: language === 'ar' ? 'يجب تحديد تاريخ الانتهاء أو اختيار متاح للأبد' : 'End date is required or select "Forever"',
          variant: 'destructive',
        });
        return;
      }
    ...
    try {
      ...
      } else if (service?.service_type === 'training') {
        finalPrice = data.is_free ? 0 : (data.price || 0);
      }
      ...
      if (service?.service_type === 'training') {
        ...
        updateData.available_forever = availableForever;
  ...
  const getFeatureKey = (): 'services' | 'trainings' | 'discounts' => {
    if (!service) return 'services';
    const type = service.service_type?.toLowerCase()?.trim();
    if (type === 'training') return 'trainings';
    if (type === 'discount' || (service.discount_percentage != null && service.discount_percentage > 0)) {
      return 'discounts';
    }
    return 'services';
  };
  ...
  if (!isTypeEnabled()) {
    return <FeatureDisabled featureKey={featureKey} />;
  }
  ...
  const durationPerSet = parseInt(form.watch("duration_per_set") || "60");
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              {service.service_type === 'training' && (
                <Card>
                  ...
                  <CardContent>
                    <TrainingAvailabilitySelector
                      availableFrom={availableFrom}
                      availableTo={availableTo}
                      availableForever={availableForever}
                      onFromChange={setAvailableFrom}
                      onToChange={setAvailableTo}
                      onForeverChange={setAvailableForever}
                    />
                  </CardContent>
                </Card>
              )}
                ...
                <Card>
                  ...
                  <CardContent>
                    <WeeklyScheduleEditor
                      ...
                      sessionDurationMinutes={durationPerSet}
                    />
                  </CardContent>
                </Card>
              )}

              {/* Dates for non-training services */}
              {(service.service_type === 'discount' || service.service_type === 'other') && (
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                          ...
                          <FormItem>
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="240">4 {isRTL ? 'ساعات' : 'hours'}</SelectItem>
                                <SelectItem value="300">5 {isRTL ? 'ساعات' : 'hours'}</SelectItem>
                                <SelectItem value="360">6 {isRTL ? 'ساعات' : 'hours'}</SelectItem>
                              </SelectContent>
                            </Select>
                            <FormMessage />
                          </FormItem>
