...
export function useUserReferralInfo() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        referral_code: gamification?.referral_code,
        referral_count: gamification?.referral_count || 0,
        is_shield_member: gamification?.is_shield_member || false,
        rewards: rewards || [],
        totalEarned: (rewards || [])
    ...
    mutationFn: async () => {
      ...
      try {
        ...
        const [bookingsResult, trainingsResult, groupsResult, referralsResult] = await Promise.all([
          ...
          supabase.from('user_gamification').select('referral_count').eq('user_id', user.id).single()
        ]);
        
        // Check for errors in parallel queries
        if (bookingsResult.error) console.error('Bookings query error:', bookingsResult.error);
        if (trainingsResult.error) console.error('Trainings query error:', trainingsResult.error);
        if (groupsResult.error) console.error('Groups query error:', groupsResult.error);
        if (referralsResult.error) console.error('Referrals query error:', referralsResult.error);
