...
export const useFriendLists = () => {
  ...
  const createList = async (name: string, description?: string, visibility: 'public' | 'private' = 'private') => {
    if (!user) return;
  ...
  const updateList = async (listId: string, name: string, description?: string, visibility?: 'public' | 'private') => {
    try {
