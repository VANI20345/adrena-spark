...
export const useFollow = (targetUserId?: string) => {
  ...
  // Follow mutation with duplicate detection
  const followMutation = useMutation({
    mutationFn: async ({ userId, isPrivate }: { userId: string; isPrivate?: boolean }) => {
      ...
      // Prevent self-follow
      if (user.id === userId) {
        throw new Error(language === 'ar' ? 'لا يمكنك متابعة نفسك' : 'You cannot follow yourself');
      }
      ...
      if (isPrivate) {
        ...
        if (existingRequest) {
          if (existingRequest.status === 'pending') {
            throw new Error(language === 'ar' ? 'طلب المتابعة معلق بالفعل' : 'Follow request already pending');
          }
          // If previously rejected, delete and create new
        ...
        if (error) {
          ...
          if (error.code === '23505') {
            throw new Error(language === 'ar' ? 'طلب المتابعة موجود بالفعل' : 'Follow request already exists');
          }
          throw error;
        }
        ...
        if (existingFollow) {
          ...
          return { type: 'already_following' as const };
        }
    ...
    onSuccess: (result, { userId }) => {
      ...
      if (result.type === 'already_following') {
        toast({
          title: language === 'ar' ? 'المتابعة موجودة' : 'Already Following',
          description: language === 'ar' ? 'أنت بالفعل تتابع هذا المستخدم.' : 'You are already following this user.',
          duration: 3000,
        });
      } else {
        toast({
