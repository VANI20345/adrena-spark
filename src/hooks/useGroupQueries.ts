  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          profiles:profiles_public!user_id(full_name, avatar_url)
        `)
        .eq('group_id', groupId);
...
export function useGroupLeaderboard(groupId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch profiles for top users in single query using public view
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: organizerGroups, error: orgError } = await supabase
        ...
        .select(`
          *,
          ...
          group_interests(
            interest_id,
            interest_categories:interest_id(name, name_ar)
          )
        `)
      ...
      const organizer = (organizerGroups || []).map(group => ({
        ...group,
        interests: (group.group_interests || []).map((gi: any) => gi.interest_categories).filter(Boolean)
      }));
      ...
      const joined = (joinedData || [])
        ...
        .map((m: any) => ({
          ...m.groups,
          memberRole: m.role,
          interests: (m.groups.group_interests || []).map((gi: any) => gi.interest_categories).filter(Boolean)
        }));
...
export function useDiscoverGroups(userId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Only exclude user's groups if logged in
      if (userId) {
    ...
    // Always enabled - anonymous users can view groups
    enabled: true,
    staleTime: 5 * 60 * 1000, // 5 minutes
    refetchOnWindowFocus: false,
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          group_id,
          profiles:profiles_public!user_id(avatar_url, full_name)
        `)
        .limit(300);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters) {
        ...
        // Member range filter (only if not default)
        if (filters.memberRange && (filters.memberRange[0] !== 0 || filters.memberRange[1] !== 500)) {
          query = query
    ...
    // Enable when there's a search term OR active filters
    enabled: searchTerm.length > 0 || hasActiveFilters,
    staleTime: 2 * 60 * 1000, // 2 minutes
    refetchOnWindowFocus: false,
  });
}
