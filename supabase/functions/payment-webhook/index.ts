  ...
  try {
    ...
    if (data.metadata?.booking_id) {
      ...
      if (type === 'payment_paid') {
        ...
        // === UNIFIED FINANCIAL CYCLE ===
        // 1. Create payment hold + 70/30 split + credit wallet + log (all atomic in RPC)
        const { data: holdResult, error: holdError } = await supabaseClient.rpc(
          'create_payment_hold_with_split',
          { p_booking_id: data.metadata.booking_id, p_booking_type: bookingType }
        );
        if (holdError) console.error('[payment-webhook] Hold creation failed:', holdError);
        else console.log('[payment-webhook] Hold created:', holdResult);

        // 2. Generate customer + provider invoices
        const { error: invErr } = await supabaseClient.rpc(
          'generate_booking_invoices',
          { p_booking_id: data.metadata.booking_id, p_booking_type: bookingType }
        );
        if (invErr) console.error('[payment-webhook] Invoice generation failed:', invErr);

        // 3. Log user payment transaction
        ...
        // 4. Log booking_payment in financial_transaction_logs (separate from hold_created)
        ...
        // 5. Notify provider (new booking received)
        if (providerId) {
          await supabaseClient.from('notifications').insert({
            user_id: providerId,
            type: 'new_booking_received',
            title: 'تم شراء تذكرة جديدة',
            message: `تم استلام حجز جديد بقيمة ${actualAmount / 100} ريال. سيتم إيداع 70% فوراً والباقي بعد فترة الحجز.`,
            data: { booking_id: data.metadata.booking_id, amount: actualAmount / 100, booking_type: bookingType }
          });
        }

        // 6. Notify all admins + super_admins
        const { data: adminRoles } = await supabaseClient
          .from('user_roles')
          .select('user_id')
          .in('role', ['admin', 'super_admin']);
        if (adminRoles && adminRoles.length > 0) {
          const adminNotifs = adminRoles.map((r: any) => ({
            user_id: r.user_id,
            type: 'admin_new_payment',
            title: 'عملية دفع جديدة',
            message: `تم استلام دفعة بقيمة ${actualAmount / 100} ريال من ${bookingType === 'event' ? 'حجز فعالية' : 'حجز خدمة'}.`,
            data: { booking_id: data.metadata.booking_id, amount: actualAmount / 100, booking_type: bookingType, provider_id: providerId }
          }));
...
async function process3DSPaymentSuccess(
  ...
  // === UNIFIED FINANCIAL CYCLE (3DS path) ===
  const { error: holdErr } = await supabaseClient.rpc('create_payment_hold_with_split', {
    p_booking_id: bookingId,
    p_booking_type: bookingType
  });
  if (holdErr) console.error('[payment-webhook][3DS] Hold creation failed:', holdErr);

  await supabaseClient.rpc('generate_booking_invoices', {
    p_booking_id: bookingId,
    p_booking_type: bookingType
  });
  ...
  // Log financial transaction (booking_payment)
  await supabaseClient.from('financial_transaction_logs').insert({
    transaction_type: 'booking_payment',
    amount: amount,
  ...
  if (finalProviderId) {
    await supabaseClient.from('notifications').insert({
      user_id: finalProviderId,
      type: 'new_booking_received',
      title: 'تم شراء تذكرة جديدة',
      message: `تم استلام حجز جديد بقيمة ${amount} ريال.`,
      data: { booking_id: bookingId, amount, booking_type: bookingType }
    });
  }
  const { data: adminRoles3ds } = await supabaseClient
    .from('user_roles').select('user_id').in('role', ['admin', 'super_admin']);
  if (adminRoles3ds && adminRoles3ds.length > 0) {
    await supabaseClient.from('notifications').insert(
      adminRoles3ds.map((r: any) => ({
        user_id: r.user_id,
        type: 'admin_new_payment',
        title: 'عملية دفع جديدة',
        message: `تم استلام دفعة بقيمة ${amount} ريال.`,
        data: { booking_id: bookingId, amount, booking_type: bookingType, provider_id: finalProviderId }
      }))
    );
  }
