...
/**
 * Unified financial model (VAT-inclusive prices):
 *   vat_amount         = total * 15 / 115
 *   net_amount         = total - vat_amount
 *   platform_commission= net * rate%
 *   commission_vat     = commission * 15%   (informational, recorded as vat_on_commission)
...
function calculateFinancialBreakdown(
  ...
  // Business model: prices VAT-inclusive. Commission is on total_amount.
  // provider_earnings = total - commission. VAT extracted is informational.
  const platformCommission = r2(totalAmount * effectiveRate / 100);
  const vatExtracted = r2(totalAmount * VAT_RATE / (1 + VAT_RATE));
  const commissionVat = r2(platformCommission * VAT_RATE / (1 + VAT_RATE));
  const providerEarnings = r2(totalAmount - platformCommission);
  return {
    totalAmount: r2(totalAmount),
    commissionRate: effectiveRate,
    platformCommission,
    vatOnCommission: commissionVat,
    netCommission: r2(platformCommission - commissionVat),
    providerEarnings,
    isDiscounted,
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Free booking bypass: no commission, no VAT, no provider earnings, no invoice, no financial log
    const isFreeBooking = totalAmount === 0;
    const financialBreakdown = isFreeBooking
      ? {
          totalAmount: 0,
          commissionRate: 0,
          ...
          netCommission: 0,
          providerEarnings: 0,
          isDiscounted: isDiscountedService,
        }
      : calculateFinancialBreakdown(totalAmount, applicableCommissionRate, isDiscountedService);
    ...
    // Create platform invoice record for ZATCA compliance (skip for free bookings)
    if (!isFreeBooking && financialBreakdown.platformCommission > 0) {
      try {
    ...
    // Log financial transaction (skip for free bookings — no money movement)
    if (!isFreeBooking) {
