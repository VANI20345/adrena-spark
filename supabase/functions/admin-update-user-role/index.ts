...
type AppRole = 'attendee' | 'provider' | 'admin' | 'super_admin';
...
serve(async (req) => {
  ...
  try {
    ...
    const validRoles: AppRole[] = ['attendee', 'provider', 'admin', 'super_admin'];
    ...
    const callerIsAdmin = callerRole?.role === 'admin' || callerIsSuperAdmin;
    ...
    // Prevent self-demotion for safety (for super_admin)
    if (userId === user.id && callerIsSuperAdmin && role !== 'super_admin') {
      return new Response(JSON.stringify({ error: 'Cannot demote your own super admin role' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      });
    }

    // Prevent demoting another super_admin (only another super_admin could attempt this)
    if (userId !== user.id) {
      ...
      if (targetRole?.role === 'super_admin' && role !== 'super_admin') {
        return new Response(JSON.stringify({ error: 'Cannot demote another super admin' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 403,
        });
      }
