  ...
  try {
    ...
    console.log('[verify-payment] Moyasar payment status:', moyasarPayment.status);
     ...
     if (moyasarPayment.status === 'paid') {
      ...
      // 1. Create payment hold + 70/30 split + credit wallet + log
      const { error: holdErr } = await serviceClient.rpc('create_payment_hold_with_split', {
        p_booking_id: booking_id,
        p_booking_type: booking_type
      });
      if (holdErr) console.error('[verify-payment] Hold creation failed:', holdErr);

      // 2. Generate invoices
      await serviceClient.rpc('generate_booking_invoices', {
        p_booking_id: booking_id,
        p_booking_type: booking_type
      });

      // Determine provider for notifications
      ...
      if (booking_type === 'service') {
        ...
        const { data: ev } = await serviceClient
          ...
          .eq('id', (booking as EventBookingRow).event_id).maybeSingle();
      ...
      await serviceClient.from('notifications').insert({
        ...
        data: { booking_id, payment_id: paymentRecord.provider_payment_id, amount: moyasarPayment.amount / 100 }
      });
      ...
      if (providerId) {
        await serviceClient.from('notifications').insert({
          ...
          message: `تم استلام حجز جديد بقيمة ${moyasarPayment.amount / 100} ريال.`,
          data: { booking_id, amount: moyasarPayment.amount / 100, booking_type }
        });
      }

      // 6. Notify admins/super_admins
      ...
      if (adminRoles && adminRoles.length > 0) {
        await serviceClient.from('notifications').insert(
          adminRoles.map((r: any) => ({
            ...
            message: `تم استلام دفعة بقيمة ${moyasarPayment.amount / 100} ريال.`,
            data: { booking_id, amount: moyasarPayment.amount / 100, booking_type, provider_id: providerId }
          }))
        );
      }
