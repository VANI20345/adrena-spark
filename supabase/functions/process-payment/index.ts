...
serve(async (req) => {
  ...
  try {
    ...
    if (Math.abs(Number((booking as any).total_amount) - amount) > 1) {
      console.error('[process-payment] amount mismatch', { booking_amount: (booking as any).total_amount, requested_amount: amount });
    ...
    const breakdown = calculateBreakdown(amount, rates[commissionType], isDiscounted);
