...
serve(async (req) => {
  ...
  try {
    ...
    // Free booking bypass: no commission, no VAT, no provider earnings
    const isFreeBooking = totalAmount === 0;
    const platformCommission = isFreeBooking ? 0 : (totalAmount * eventCommissionRate) / 100;
    const organizerEarnings = isFreeBooking ? 0 : totalAmount - platformCommission;
    if (isFreeBooking) {
      vatAmount = 0;
    }
    ...
    // For paid events, set to 'pending_payment' with reservation expiry
