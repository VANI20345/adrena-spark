  ...
  try {
    ...
    if (pricing_plan_id) {
      ...
      if (plan.ticket_limit < quantity) {
        throw new Error(`Ticket quantity (${quantity}) exceeds the selected pricing plan limit (${plan.ticket_limit})`);
      }
    ...
    console.error('Booking creation error:', err?.message || err, err?.stack);
    return new Response(JSON.stringify({
