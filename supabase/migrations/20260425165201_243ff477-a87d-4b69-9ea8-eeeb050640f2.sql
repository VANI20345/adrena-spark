...
AS $$
  ...
  role_counts AS (
    ...
    INNER JOIN profile_users pu ON pu.user_id = ur.user_id
  ),
  organizer_counts AS (
    SELECT
      COUNT(DISTINCT e.organizer_id) AS event_organizers
    FROM public.events e
    INNER JOIN profile_users pu ON pu.user_id = e.organizer_id
  ),
  group_leader_counts AS (
    SELECT
      COUNT(DISTINCT g.created_by) AS group_leaders
    FROM public.groups g
    INNER JOIN profile_users pu ON pu.user_id = g.created_by
  ),
  totals AS (
    SELECT COUNT(*) AS total_users FROM profile_users
  )
  SELECT
