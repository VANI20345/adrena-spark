...
BEGIN
  BEGIN
    INSERT INTO public.friend_activities (user_id, activity_type, event_id, activity_data)
...
BEGIN
  ...
  VALUES (v_user_id,'payment',-v_total,
  ...
  IF p_booking_type='event' AND v_points_used>0 AND NOT EXISTS (
    SELECT 1 FROM public.loyalty_ledger WHERE reference_id=p_booking_id AND reference_type='event_booking' AND type='redeemed'
  ) THEN
    INSERT INTO public.loyalty_ledger (user_id,points,type,description,reference_id,reference_type)
    VALUES (v_user_id,-v_points_used,'redeemed','نقاط مستخدمة',p_booking_id,'event_booking');
  END IF;
...
BEGIN
  ...
  IF v_available>0 THEN
    INSERT INTO public.wallet_transactions (user_id,type,amount,description,status,reference_id,reference_type)
    VALUES (v_provider_id,'earning',v_available,
