...
BEGIN
  ...
  IF v_is_lp THEN
    -- Landing page flow: try both raw code and LP-prefixed code in form_submissions
  ...
  ELSE
    -- Friend referral flow: try user_gamification first
    SELECT * INTO v_referrer_row FROM user_gamification WHERE referral_code = v_clean_code;
    
    IF NOT FOUND THEN
      -- FALLBACK: check form_submissions for raw landing-page codes (without LP- prefix)
      SELECT * INTO v_form_row FROM form_submissions 
      WHERE referral_code = v_clean_code OR referral_code = 'LP-' || v_clean_code
      LIMIT 1;
      
      IF FOUND THEN
        -- It's actually a landing page code used without LP- prefix
        IF v_form_row.referral_used THEN
          RETURN jsonb_build_object('ok', false, 'code', 'already_used');
        END IF;
